# Unit test: Heider (2021) normal-strain permeability, isotropic fallback,
# axis-aligned damage gradient.
#
# Geometry: single QUAD4 of size 1 x 1 m on [0,1]^2.
# Damage profile: d(x,y) = 0.5 + 0.4 * x   -> grad(d) = (0.4, 0, 0), n_d = (1,0,0).
# Strain:        disp_x = 1e-3 * x, disp_y = 0 -> eps_xx = 1e-3, others = 0.
# eps_nn = n_d . eps . n_d = eps_xx = 1e-3.
# h_c    = 1e-3 (constant).
# w_c    = h_c * |1 + eps_nn| = 1e-3 * 1.001.
# chi_d  = 1 everywhere since d in [0.5, 0.9].
# k_w    = w_c^2 / 12.
# d_qp^b at the single QP: with QUAD4 default 2x2 Gauss, d at QP ~= 0.5 + 0.4*0.5 = 0.7
# so weight ~= 0.7^2 = 0.49.
# Expected (isotropic fallback):
#   K = k0 * I + 0.49 * k_w * I  (identity times scalar).
...
[Variables]
  # A trivial variable so the Steady solve is non-empty.
  [u]
  []
[]
