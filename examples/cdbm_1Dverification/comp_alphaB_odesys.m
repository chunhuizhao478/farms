...
if case_index == '0'
f_out = [ ( 1 - y(2) ) * ( param.C_1 * exp( y(1) / param.C_2 ) * var.I2 * ( var.xi - param.xi_0 ) ) 
           %( param.C_1 * exp( y(1) / param.C_2 ) * var.I2 * ( var.xi - param.xi_0 ) ) 
           param.C_BH * ( var.I2 * (mu_new - gamma_new * var.xi + lambda_new/2 * var.xi ^ 2) - var.I2 * (param.a0 + param.a1 * var.xi + param.a2 * var.xi ^ 2 + param.a3 * var.xi ^ 3)) ];
         
end

if case_index == '1' 
f_out = [ ( 1 - y(2) ) * ( param.C_1 * exp( y(1) / param.C_2 ) * var.I2 * ( var.xi - param.xi_0 ) ) 
           param.C_B * Prob * ( 1 - y(2) ) * ( var.I2 * (mu_new - gamma_new * var.xi + lambda_new/2 * var.xi ^ 2) - var.I2 * (param.a0 + param.a1 * var.xi + param.a2 * var.xi ^ 2 + param.a3 * var.xi ^ 3)) ];
           %( param.C_1 * exp( y(1) / param.C_2 ) * var.I2 * ( var.xi - param.xi_0 ) )  
