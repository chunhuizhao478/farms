...
# mu_s = 0.677
# S = ( mu_s - mu ) / ( mu - mu_d ) = ( 0.677 - 0.583 ) / ( 0.583 - 0.1) = 0.194
# Frictional Length Scale L = G Dc / ( ( mu_s - mu_d ) sigma_yy ) = 32.04e9 * 0.2 / (( 0.677 - 0.1) * 120e6) = 92m
# Use mesh size = 10m
##########################################################
...
[Mesh]
    [./msh]
        ...
        file =  '../../meshgenerator/cdbm/frankcase/contmf.msh'
    []
    [./new_block_1]
      type = ParsedSubdomainMeshGenerator
      input = msh
      combinatorial_geometry = 'x >= -600 & x<= -200 & y<=50 & y>=40'
      block_id = 1
    []
    [./new_block_2]
        type = ParsedSubdomainMeshGenerator
        input = new_block_1
        combinatorial_geometry = 'x >= -600 & x<= -200 & y<=40 & y>=30'
        block_id = 2
    []
    [./new_block_3]
      type = ParsedSubdomainMeshGenerator
      input = new_block_2
      combinatorial_geometry = 'x >= 200 & x<= 600 & y<= -30 & y>= -40'
      block_id = 3
    []
    [./new_block_4]
        type = ParsedSubdomainMeshGenerator
        input = new_block_3
        combinatorial_geometry = 'x >= 200 & x<= 600 & y<=-40 & y>=-50'
        block_id = 4
    []
    [./split]
  ...
  [MultiApps]
    [./sub_app]
        ...
        positions = '0 0 0'
        input_files = 'test_frankcase_sub.i'
        execute_on = 'TIMESTEP_BEGIN'
    [../]
  []
