...
[GlobalParams]
    ...
    V_o = 1e-12  # Reference slip rate
    Vini = 1e-9  # Initial slip rate
    f_o = 0.6    # Initial friction coefficient
    a = 0.008    # Direct effect parameter
    b = 0.012    # State variable evolution parameter
    L = 0.02     # State variable characteristic distance
    statevarini = 1e9  # Initial state variable 
    ...
    background_normal_stress = 120e6

    ##initial shear traction (Pa)
    background_tangential_stress = 75e6
[]
...
[Executioner]
    ...
    [TimeStepper]
        ...
        cp = 6000                # P-wave velocity
        a_prem = 0.1             # Proportionality index for stable time step
        shear_modulus = 32.04e9  # Shear modulus
        permeability = 1e-15     # Permeability value (k_py)
        biot_modulus = 1.0e-11   # Inverse of Biot modulus (c_o)
        a_o = 0.008              # Direct effect parameter
        b_o = 0.012              # State evolution parameter
        L = 0.02                 # Characteristic slip distance
        normal_stress = 120e6    # Normal stress on the fault
