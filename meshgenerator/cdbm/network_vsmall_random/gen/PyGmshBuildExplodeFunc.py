...
def gmsh_add_points_2nd(array_coord, 
    ...
    lc2: mesh specified at point (inner network)

    lc3: outside boundary
   ...
   well:
    ...
    for surf_point_ind in range(np.shape(mat_surf_ptr)[0]):
        ...
        # add gmsh entity
        if meshtype == "createfine":
            xperturb = random.SystemRandom().randint(-50, 50)
            yperturb = random.SystemRandom().randint(-50, 50)
            ptr_i = gmsh.model.occ.addPoint(surf_x_i + xperturb,surf_y_i + yperturb,0,lc2)
        else:
            xperturb = random.SystemRandom().randint(-50, 50)
            yperturb = random.SystemRandom().randint(-50, 50)
            ptr_i = gmsh.model.occ.addPoint(surf_x_i + xperturb,surf_y_i + yperturb,0,lc3)
...
def gmsh_add_elems_2nd(mat_connect_corner, 
   ...
   [5] [6] [7]
    ...
    #--------------------------------------Store the information for each segment---------------------------------------------#
    #!This function runs two times in the PyGmshFineMeshGenMesh.py, the last run is what we want!
    #generate a array to store information
    #pointA x, pointA y, pointB x, pointB y, local shear stress, local normal stress, angle, mu, S
    segments_info_array = np.zeros((3*np.shape(mat_connect_lineendptrs)[0],9))
    segments_info_count = 0
    #need to synchronize before looking up the point
    gmsh.model.occ.synchronize()
    #global stress field [NEED TO DOUBLE CHECK!]
    print("Please make sure the global stress field (gmsh_add_elems_2nd) is update to date to obtain the correct segment info ...")
    input()
    ...
    for tria_elem_ind in range(np.shape(mat_connect_lineendptrs)[0]):
        ...
        for index in [0, 1, 2]:
            ...
            #get points of lines (gmsh index)
            ptrA = tria_elem_data[2 * index]; ptrB = tria_elem_data[2 * index + 1];
        
            #convert to physical coordinates
            #Reference: http://onelab.info/pipermail/gmsh/2019/013326.html 
            no_parametrization = []
            [xA, yA, zA] = gmsh.model.getValue(0, ptrA, no_parametrization)
            [xB, yB, zB] = gmsh.model.getValue(0, ptrB, no_parametrization)
            ...
            #store info
            segments_info_array[segments_info_count,:] = [xA, yA, xB, yB, local_shear_sts, local_normal_sts, angle, mu, S]

            #update count
            segments_info_count += 1

            # # # will print 1.0, 1.0, 1.0
            # print("xA: {}, yA: {}, zA: {}".format(xA, yA, zA))
            # print("xA: {}, yA: {}, zA: {}".format(xB, yB, zB))
            # print("local_shear_sts: {}, local_normal_sts: {}, S: {}".format(local_shear_sts, local_normal_sts, S))
            # segments_info_array = segments_info_array[~np.all(segments_info_array == 0, axis=1)]
            # segments_info_array = np.unique(segments_info_array, axis=0)
            # np.savetxt("../outputs/segment_info_array.txt",segments_info_array, fmt="%.3f")
            # exit()
...
def gmsh_add_physical_entities_2nd(arr_elem_corner, 
   ...
   [5] [6] [7]
    ...
    if case_flag == "2D-Cluster":
        ...
        for surface_id in range(np.shape(arr_surface_nw)[0]):
            gmsh.model.addPhysicalGroup(2,[arr_surface_nw[surface_id,0]],name="surfnet" + str(arr_surface_nw[surface_id,0]))
            #sabe in mat_surf_phyname
            mat_surf_phyname[arr_surface_nw[surface_id,0]] = "surfnet" + str(arr_surface_nw[surface_id,0])
    # if case_flag == "2D-Cluster-Well":
    #     for additional_surface_id in range(np.shape(arr_additional_surface["circle"])[0]):
    #         gmsh.model.addPhysicalGroup(2,[arr_additional_surface["circle"][additional_surface_id,0]],name="surfwell" + str(arr_additional_surface["circle"][additional_surface_id,0]))
    #         #sabe in mat_surf_phyname
...
def meshio_generate_subdomainID(file_path2,
    ...
    #brute-force generates block pairs (assuming i ~ all other j)
    #the correct ones will be determined using "meshcheck & block_pair_extractor.py"
    list_subdomain_surrounding_copy = list_subdomain_surrounding.copy()
    list_subdomain_surrounding_copy.insert(0, 0) #insert 0
    list_subdomain_block_pairs = []
    for i in range(len(list_subdomain_surrounding_copy)):
        for j in range(i,len(list_subdomain_surrounding_copy)):
            if i < j:
                subdomainID_i = list_subdomain_surrounding_copy[i]
                subdomainID_j = list_subdomain_surrounding_copy[j]
                list_subdomain_block_pairs.append(str(subdomainID_i)+" "+str(subdomainID_j)+";")
    ...
    np.savetxt(txt_block_pairs,list_subdomain_block_pairs,fmt='%s',newline=" ")
...
# GMSH function to create surface given line loop
def gmsh_add_surfaces_fine_mesh(mat_surface_lineloop,
                                mat_surface_additional_lineloop,
                                case_flag,
                                arr_elem_trialoop):
...
# GMSH function to create physical entities
def gmsh_add_physical_entities_fine_mesh(arr_elem_corner, 
                                        arr_elem_additional,
                                        arr_surface,
                                        arr_additional_surface,
                                        case_flag):
...
# Save Network Ptrs/Connectivity
def meshio_save_network_info_fine_mesh(file_path1,
                                        array_coord,
                                        dist_verticalbc,
                                        dist_lateralbc,
                                        case_flag,
    ...
    #Define Larger mat for store connectivity [ptr_i_x, ptr_i_y, ptr_j_x, ptr_j_y]
    mat_surf1_connect = np.zeros((np.shape(tria_elem_connect)[0] * 3,4))
    mat_surf1_connect_count = 0

    #Define Larger mat for store triangle three coordinates
    #[ptr_i_x, ptr_i_y, ptr_j_x, ptr_j_y, ptr_k_x, ptr_k_y]
    mat_surf1_triacoordall = np.zeros((np.shape(tria_elem_connect)[0] * 3,6))
    mat_surf1_tria_count = 0
    ...
    #Loop over tria
    for tria_ind in range(np.shape(tria_elem_connect)[0]):
        
        #get connectivity for current element
        tria_connect_i = tria_elem_connect[tria_ind,:]
