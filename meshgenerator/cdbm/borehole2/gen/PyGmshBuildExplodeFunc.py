...
def gmsh_add_elems_2nd(mat_connect_corner, 
   ...
   [5] [6] [7]
    ...
    for tria_elem_ind in range(np.shape(mat_connect_lineendptrs)[0]):
        ...
        #check if the boundary element of borehole (assume the center of circle is (0,0))
        dist_k1 = np.sqrt((tria_elem_data[0])**2 + (tria_elem_data[1])**2)
        dist_k2 = np.sqrt((tria_elem_data[2])**2 + (tria_elem_data[3])**2)
        dist_k3 = np.sqrt((tria_elem_data[4])**2 + (tria_elem_data[5])**2)

        flag = 0
        if abs( dist_k1 - radius ) < 1e-3: ptr1_coords = mat_surf_trialinesegmentall[tria_elem_ind, 0:2]; ptr2_coords = mat_surf_trialinesegmentall[tria_elem_ind, 2:4]; flag = 1
        if abs( dist_k2 - radius ) < 1e-3: ptr1_coords = mat_surf_trialinesegmentall[tria_elem_ind, 4:6]; ptr2_coords = mat_surf_trialinesegmentall[tria_elem_ind, 6:8]; flag = 1
        if abs( dist_k3 - radius ) < 1e-3: ptr1_coords = mat_surf_trialinesegmentall[tria_elem_ind, 8:10]; ptr2_coords = mat_surf_trialinesegmentall[tria_elem_ind, 10:12]; flag = 1

        if flag == 1:

            vec = ptr2_coords - ptr1_coords
