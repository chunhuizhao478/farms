...
# -------------------------------------------------------------------
# SHEAR STRENGTHS (for total stress, used in total stress plot)
# -------------------------------------------------------------------
static_shear_strength_total = c + np.abs(mu_s * (sigma_yy_total + Pf))
residual_shear_strength_total = c + np.abs(mu_d * (sigma_yy_total + Pf))

# -------------------------------------------------------------------
# Combined figure with seismic properties, total stress, effective stress, and strain invariants
# -------------------------------------------------------------------
...
# For case2f, we'll use constant seismic properties similar to case1f
...
# -------------------------------------------------------------------
# Second subplot: Total Stress Components with Pore Pressure
# -------------------------------------------------------------------
ax2.plot(np.abs(sigma_zz_total) / 1e6, depths / 1e3, label=r"$\sigma_{zz}$")
ax2.plot(np.abs(sigma_xx_total) / 1e6, depths / 1e3, label=r"$\sigma_{xx}$")
ax2.plot(np.abs(sigma_yy_total) / 1e6, depths / 1e3, label=r"$\sigma_{yy}$")
ax2.plot(sigma_xy_total / 1e6, depths / 1e3, label=r"$\sigma_{xy}$")
ax2.plot(Pf / 1e6, depths / 1e3, label="Pore Pressure", linestyle="--", alpha=0.8)
ax2.plot(
    static_shear_strength_total / 1e6,
    depths / 1e3,
...
ax2.plot(
    residual_shear_strength_total / 1e6,
    depths / 1e3,
