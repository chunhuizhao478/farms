"""
mpirun -np 8 python3 correlated_vonkarman_2D.py \
  --nx 1024 --ny 256 \
...
def parallel_interpolate_and_write(comm, x, y, field, exo_path, out_csv, plot=False,
                                   mean_val=0.1, std_val=0.025, plot_prefix="mapped_vonkarman",
    ...
    interp_linear = RegularGridInterpolator((y, x), field, bounds_error=False, fill_value=np.nan)
    interp_nearest = None
    if oob_mode == 'nearest':
    ...
    if oob_mode == 'nan':
        ...
        pts_clipped = np.column_stack((np.clip(recv_y, y[0], y[-1]), np.clip(recv_x, x[0], x[-1])))
    ...
    if taper_width and taper_width > 0:
        ...
        fx[mask_r] = 0.5 * (1.0 + np.cos(np.pi * (recv_x[mask_r] - x_max_bound) / taper_width))
        # Left side (x_min - w .. x_min)
        mask_l = (recv_x < x_min_bound) & (recv_x >= x_min_bound - taper_width)
        fx[mask_l] = 0.5 * (1.0 + np.cos(np.pi * (x_min_bound - recv_x[mask_l]) / taper_width))
        # Beyond taper -> 0
        ...
        fy[mask_t] = 0.5 * (1.0 + np.cos(np.pi * (recv_y[mask_t] - y_max_bound) / taper_width))
        mask_b = (recv_y < y_min_bound) & (recv_y >= y_min_bound - taper_width)
        fy[mask_b] = 0.5 * (1.0 + np.cos(np.pi * (y_min_bound - recv_y[mask_b]) / taper_width))
        ...
        in_box = (recv_x >= x_min_bound) & (recv_x <= x_max_bound) & \
                 (recv_y >= y_min_bound) & (recv_y <= y_max_bound)
        if np.any(~in_box):
    ...
    if rank == 0:
        ...
        if plot:
            try:
                ...
                c1 = ax1.contourf(x, y, mean_val + std_val * field, levels=50, cmap='viridis')
                ...
                sc = ax2.scatter(x_coord[mask], y_coord[mask], c=mapped[mask], s=5, cmap='viridis')
...
def main():
    parser = argparse.ArgumentParser(description="MPI von Kármán field generator and mapper")
    ...
    parser.add_argument('--corr-len', type=float, default=200, help='Correlation length (m)')
    ...
    parser.add_argument('--std', type=float, default=0.025, help='Std dev of damage variable')
    parser.add_argument('--exo', type=str, default='./static_solve_out.e', help='Exodus/NetCDF file')
    parser.add_argument('--out', type=str, default='./mapped_vonkarman_field.csv', help='Output CSV path')
    parser.add_argument('--seed', type=int, default=None, help='Random seed')
    parser.add_argument('--plot', action='store_true', help='Enable plotting and save PNGs on rank 0')
    parser.add_argument('--plot-prefix', type=str, default='mapped_vonkarman', help='Prefix for saved plot files')
    parser.add_argument('--oob', type=str, default='nearest', choices=['nearest','nan','clip'],
    ...
    parallel_interpolate_and_write(
        ...
        mean_val=args.mean, std_val=args.std,
