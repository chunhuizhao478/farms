...
#Switches based on maximum velocity threshold (1e-2 m/s)
[Mesh]
    [./msh]
...
[Kernels]
    ...
    # DYNAMIC MODE: Inertia terms (disabled initially, enabled when max velocity > 1e-2 m/s)
    ...
    # Inner region (blocks 1, 3): enabled initially, disabled when max velocity > 1e-2 m/s
    [rad_damp_x_inner]
...
[Postprocessors]
    ...
    [maxvelmag]
        ...
        variable = vel_mag
    []
    # KEY: Deviatoric strain rate monitoring (for output only)
    [max_dev_strain_rate]
    ...
    [adaptive_dt_bound]
        ...
        criterion_postprocessor = maxvelmag
        threshold = 1e-2
        comparison_type = greater_than
        # When max velocity > 1e-2 m/s (dynamic mode): use small dt_max
        dt_bound_above_threshold = 1e-2
        # When max velocity <= 1e-2 m/s (quasi-dynamic mode): use large dt_max
...
[Controls]
    ...
    # BIDIRECTIONAL SWITCHING: Based on maximum velocity
    # When max(velocity) > 1e-2 m/s: Switch ALL REGIONS to DYNAMIC mode
    # When max(velocity) <= 1e-2 m/s: Switch ALL REGIONS to QUASI-DYNAMIC mode
    [velocity_switch]
        type = FarmsConditionalPostprocessorEnableControl
        postprocessor = maxvelmag
        comparison_type = greater_than
        threshold = 1e-2
        reverse_on_false = true

        # Enable when max velocity > 1e-2 m/s (DYNAMIC mode in ALL regions)
        enable_objects = 'Kernels/inertia_x Kernels/inertia_y
                          AuxKernels/accel_x_aux AuxKernels/accel_y_aux
                          AuxKernels/vel_x_aux_dynamic AuxKernels/vel_y_aux_dynamic'

        # Disable when max velocity > 1e-2 m/s (turn off QUASI-DYNAMIC in ALL regions)
