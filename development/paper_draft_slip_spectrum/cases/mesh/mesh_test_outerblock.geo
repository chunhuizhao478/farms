...
// ====================================================================
// IMPROVED: Distance-based mesh gradation from initial damage box
// Mesh size increases smoothly from lc_fault (100) at damage box
// to lc (2000) at domain boundaries, independent of block boundaries
// ====================================================================

// 1. Create a Distance field from the initial damage box boundary lines
Field[1] = Distance;
Field[1].CurvesList = {9, 10, 11, 12};  // Lines defining damage box boundary
Field[1].Sampling = 100;  // Number of points for distance calculation

// 2. Create a Threshold field for smooth mesh size transition
// Uses distance from damage box to control mesh size
Field[2] = Threshold;
Field[2].IField = 1;
Field[2].LcMin = lc_fault;           // 100 m at damage box
