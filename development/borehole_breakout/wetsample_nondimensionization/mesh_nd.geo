...
lc_min = 0.01;
lc_max = 0.05; // Maximum mesh size

// Define main cylinder
// Point(1) = {0, 0, 0, lc};
// Point(2) = {0, 0, 0.105, lc};
Cylinder(1) = {0, 0, 0, 0, 0, 1.05, 0.27, 2*Pi};
...
Cylinder(2) = {0, 0, 1.05 - h_cut, 0, 0, h_cut, r_cut, 2*Pi};
...
Cylinder(5) = {-0.35, 0, 0.525, 2, 0, 0, 0.055, 2*Pi};
...
Field[1] = Cylinder;
Field[1].XCenter = -0.35;         // Center of the cylinder (same as Cylinder(5))
Field[1].YCenter = 0;
Field[1].ZCenter = 0.525;
Field[1].XAxis   = 1;              // Orientation: Cylinder(5) is created along the x-axis
Field[1].YAxis   = 0;
Field[1].ZAxis   = 0;
Field[1].Radius  = 0.15;         // Same radius as Cylinder(5)
