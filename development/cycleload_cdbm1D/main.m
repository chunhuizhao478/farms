...
%       %{
      ...
      if var.Prob == 0 %%var.alpha_ < alpha_cr_new 
          fprintf("simulaton time: %.4f, xi: %.10f, alpha: %.10f, B: %.10f, alpha_cr: %.10f eps_p %.10f, eps: %.10f, Prob: %d, state: %s \n", ...
                  var.t,var.xi,var.alpha_,var.B,alpha_cr_new,var.eps_p(1,1),var.eps(1,1), var.Prob, "solid")
      elseif var.Prob > 0 %%(var.alpha_ >= alpha_cr_new && var.alpha_ <= 1)
          fprintf("simulaton time: %.4f, xi: %.10f, alpha: %.10f, B: %.10f, alpha_cr: %.10f eps_p %.10f  eps: %.10f, Prob: %d, state: %s \n", ...
                  var.t,var.xi,var.alpha_,var.B,alpha_cr_new,var.eps_p(1,1),var.eps(1,1), var.Prob, "granular")
      else
...
plot_figure(var);
%% Save data %%
save_data(var,given_total_strain);
disp("All done!")
