 ...
 * with transition_length = 4 km (extended from 2 km).
 */
...
Fault_width = 20e3;
Fault_dip = 90*Pi/180.;
transition_length = 4e3; // 4 km halo around fault (extended from 2 km)
...
// Adjusted coefficients to maintain similar mesh size at extended boundary (4km vs 2km)
Field[2] = MathEval;
Field[2].F = Sprintf("0.05*F1 +(F1/5e3)^2 + %g", lc_fault);
