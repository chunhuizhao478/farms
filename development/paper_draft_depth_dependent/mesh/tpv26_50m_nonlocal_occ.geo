...
// Additional Box refinement field: 50 m inside box, 200 m outside
Field[8] = Box;
Field[8].XMin = -0.5 * Fault_length - refined_length;
Field[8].XMax =  0.5 * Fault_length + refined_length;
Field[8].YMin =  0.0 - refined_length;
Field[8].YMax =  0.0 + refined_length;
Field[8].ZMin = -1.0 * Fault_width - refined_length; // extend slightly deeper
Field[8].ZMax =  0.0;                     // top surface
Field[8].VIn  = 50;    // target size inside box
Field[8].VOut = lc;   // target size outside box
