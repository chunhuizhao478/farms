...
# Physics setup
# ---------------------------------------------------
# Over-pressurization parameters
...
# Bulk rock density for lithostatic overpressure (use final value from profile)
rho = rho_knots[-1]  # kg/m^3 (= 2900 at 15 km depth)
...
# -------------------------------------------------------------------
# PORE PRESSURE (piecewise-analytic with over-pressurization)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# VERTICAL STRESS (overburden from depth-varying rock density)
# -------------------------------------------------------------------
...
# Assemble stress tensor components (using effective stress)
stress = np.zeros((len(depths), 3, 3))
stress[:, 0, 0] = sigma_xx_eff
stress[:, 1, 1] = sigma_yy_eff
stress[:, 2, 2] = sigma_zz_eff
stress[:, 0, 1] = sigma_xy
stress[:, 1, 0] = sigma_xy
...
# ------------------------
# Second subplot: Effective Stress Components
# ------------------------
ax2.plot(np.abs(sigma_zz_eff) / 1e6, depths / 1e3, label=r"$\sigma_{zz}'$")
ax2.plot(np.abs(sigma_xx_eff) / 1e6, depths / 1e3, label=r"$\sigma_{xx}'$")
ax2.plot(np.abs(sigma_yy_eff) / 1e6, depths / 1e3, label=r"$\sigma_{yy}'$")
