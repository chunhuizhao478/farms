...
def stress_transformation(sigmaxx, sigmaxy, sigmayy, angle):
    ...
    local_normal_sts = (
        0.5 * (sigmaxx + sigmayy)
        - 0.5 * (sigmaxx - sigmayy) * np.cos(2 * np.radians(angle))
        + sigmaxy * np.sin(2 * np.radians(angle))
    )

    # local shear stress
    local_shear_sts = -0.5 * (sigmaxx - sigmayy) * np.sin(
        2 * np.radians(angle)
    ) + sigmaxy * np.cos(2 * np.radians(angle))
...
def main_func(
    sigmayy, nu, added_normalsts, sigmayy_label, added_normalsts_label, mesh_path
):
    ...
    slope, slope_prep, b_30, b_start_30, b_end_30 = find_prep_line_eq(
        start_ptr_x, start_ptr_y, end_ptr_x, end_ptr_y
    )
    ...
    for elem_ind in range(num_elem):
        elem_connect_i = tria_elem_connect[
            elem_ind, :
        ]  # get connectivity for current element
        num_ptr_collapse = np.count_nonzero(
            np.isin(phy_nodal_arr_30, elem_connect_i)
        )  # count
        if num_ptr_collapse >= 1:  # this is a crack aligned element
            ...
            if (
                len(
                    np.argwhere(
                        slope_prep * coord_data_x + b_start_30 - coord_data_y >= 0
                    )
                )
                > 1
                and len(
                    np.argwhere(
                        slope_prep * coord_data_x + b_end_30 - coord_data_y <= 0
                    )
                )
                > 1
            ):  # slope_prep > 0
                if (
                    slope * np.sum(coord_data_x) / 3 + b_30 - np.sum(coord_data_y) / 3
                    < 0
                ):  # hardcode #above the fault #center ptr
                ...
                if np.count_nonzero(arr_elem_properties[elem_ind, :]) == 0:
                    ...
                    x_max_lim = (174.920 + 8.0) * 1e-3
                    x_min_lim = (174.920 - 8.0) * 1e-3
                    y_max_lim = (133.648 + 4.5) * 1e-3
                    y_min_lim = (133.648 - 4.5) * 1e-3
                    if (
                        np.sum(coord_data_x) / 3 <= x_max_lim
                        and np.sum(coord_data_x) / 3 >= x_min_lim
                        and np.sum(coord_data_y) / 3 <= y_max_lim
                        and np.sum(coord_data_y) / 3 >= y_min_lim
                    ):
    ...
    ind_elem_all_embeded = (
        ind_elem_all_embeded1 + ind_elem_all_embeded2 + ind_elem_all_embeded3
    )
    ...
    output_dir = (
        Path("./case_P" + str(sigmayy_label))
        / ("case_deltasigma" + str(added_normalsts_label))
    )
    output_dir.mkdir(parents=True, exist_ok=True)

    # save elem ids
    np.savetxt(
        output_dir / "list_elemid_embeded.txt",
        ind_elem_all_embeded,
        fmt="%i",
        newline=" ",
    )
    ...
    # save subdomain id
    np.savetxt(
        output_dir / "list_subdomainid_embeded.txt",
        ind_subdomainid_all_embeded,
        fmt="%i",
        newline=" ",
    )
    ...
    with open(
        output_dir / "faults_elem_properties.csv",
        "w",
        encoding="UTF8",
        newline="",
    ) as f:
