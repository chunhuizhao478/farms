...
#increase the normal stress by 3MPa
sigma_N = sigma_N - 3.0e6
...
#save elem ids
np.savetxt('./outputs/list_elemid_embeded.txt',
            ind_elem_all_embeded,
            fmt='%i',
            newline=" ")

#save subdomain id
np.savetxt('./outputs/list_subdomainid_embeded.txt',
            ind_subdomainid_all_embeded,
            fmt='%i',
            newline=" ")

#save property elem csv
with open("./outputs/faults_elem_properties.csv", 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
