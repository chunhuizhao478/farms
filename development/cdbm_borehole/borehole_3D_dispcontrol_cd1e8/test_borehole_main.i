...
[Functions]
  ...
  [func_strain_zz]
    ...
    execute_on = 'INITIAL'
  [../]
  #set loading strain rate 3.3e-4 /s
  #3.3*1e-4*0.075 = 2.475e-5 m/s, each face 2.475e-5/2 = 1.2375e-5 m/s
  [func_bc_top]
    type = ParsedFunction
    expression = '-1.2375e-5 * t'
  []
  [func_bc_bottom]
    type = ParsedFunction
    expression = '1.2375e-5 * t'
...
[BCs]
  [disp_top]
    ...
    boundary = top
    variable = disp_y
    function = func_bc_top
  []
  [disp_bottom]
    type = FunctionDirichletBC
    boundary = bottom
    variable = disp_y
    function = func_bc_bottom
  []
[]
