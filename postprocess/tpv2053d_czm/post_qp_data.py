...
input_path = "./farms_data/"
output_path = "./farms_data_elemental/"
...
def find_closest_index(df, target):
    ...
    dx = df['x'] - target[0]
    dy = df['y'] - target[1]
    dz = df['z'] - target[2]  # Assuming z is also a coordinate in the DataFrame
    # Use squared distance for efficiency
    dist2 = dx**2 + dy**2 + dz**2

    # Check if the distance series is not empty and contains non-NaN values
    if dist2.dropna().empty:
        raise ValueError("No valid numeric entries found in 'x' and 'y' and 'z' columns to compute distances.")
...
df_first['z'] = pd.to_numeric(df_first['z'], errors='coerce')
...
def make_output_filename(quantity, point, faultname):
    """
    ...
    dip_km = point[2] / 1000.0  # Assuming z is the depth coordinate
    strike_str = f"{strike_km:.1f}"  # One decimal place
    dip_str = f"{dip_km:.1f}"  # One decimal place

    if faultname == "branch_fault":
        return f"{quantity}_branch_strike{strike_str}_dip{dip_str}.txt"
...
for key, ts in data_series.items():
    pt, qty = key
    out_fname = make_output_filename(qty, pt, faultname)
