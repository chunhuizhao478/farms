...
def CalcPlasticWork(save_folder_output_file_path):
    ...
    #
    arr_param_1 = nc.variables['vals_elem_var'+str(var_ind)+'eb'+str(1)]
    arr_param_2 = nc.variables['vals_elem_var'+str(var_ind)+'eb'+str(2)]

    #
    print(np.shape(arr_param_1))
    print(np.shape(arr_param_2))

    arr_param_sum_1 = np.sum(arr_param_1,axis=1)
    arr_param_sum_2 = np.sum(arr_param_1,axis=1)
    arr_param_sum = arr_param_sum_1 + arr_param_sum_2

    print(np.shape(arr_param_sum))
    print(np.max(arr_param_1[:21,:]))

    plt.figure()
    plt.plot(arr_param_sum[:21])

    np.savetxt("./outputs/plastic_work/plastic_work.txt",arr_param_sum[:21])
...
if __name__ == "__main__":
    ...
    exodus_file_path = "/Volumes/One Touch/Research/DamageBreakage/planarfaulttests/linearelastic/s02/test_planarfault_main_out.e"
