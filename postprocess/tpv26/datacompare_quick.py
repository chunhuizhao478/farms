...
benchmark_path = "./benchmark_data/finitedifference_strike"+str(xcoord_i)+"_dip"+str(zcoord_i)+".txt"
farms_path = "./farms_data/strike"+str(xcoord_i)+"_dip"+str(zcoord_i)+".txt"
...
plt.plot(farms[:,0],2*farms[:,1],'b--',label=farms_label)
plt.title(benchmark_code+" slip time history at strike "+str(xcoord_i)+"km and at dip "+str(zcoord_i)+"km ")
plt.legend()
plt.xlabel("time (s)")
plt.ylabel("slip (m)")
plt.savefig("./results/slip_strike"+str(xcoord_i)+"_dip"+str(zcoord_i)+".png")
plt.show()

## slip rate
plt.figure()
plt.plot(benchmark[:,0],benchmark[:,2],'r-',label=benchmark_label)
plt.plot(farms[:,0],2*farms[:,2],'b--',label=farms_label)
...
plt.savefig("./results/sliprate_strike"+str(xcoord_i)+"_dip"+str(zcoord_i)+".png")
plt.show()   