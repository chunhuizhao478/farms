...
def PlotPtrXYVal(nc,save_folder_output_file_path,plot_var_name,ptr_coord,angle,i,showfig=False):
    ...
    plt.plot(timeseries[:],ptr_x_valhist[:]/1e6,'b-',label = "pressure")
    ...
    plt.legend()
    plt.title("Time History of Velocity at injection location: "+str(ptr_x)+" , "+str(ptr_y))
    # plt.xlim([0,35])
    # plt.ylim([0,18.0])
    ...
    np.savetxt(save_folder_output_file_path+'/outputs/list_timeseries'+str(i)+'.txt',
                    ...
                    fmt='%.7f',
                    newline=" ")
    np.savetxt(save_folder_output_file_path+'/outputs/list_pressure'+str(i)+'.txt',
                    ptr_x_valhist[:],
                    fmt='%.7f',
                    newline=" ")
...
exodus_file_path = "/Volumes/One Touch/Research/DamageBreakage/porepressure/network/Dec30_utahforge/test_network_main_out.e"
save_folder_output_file_path = "."
...
#plot variable names
plot_var_name = ["pressure"]

#redo get val op
get_val_op = False

given_coord_list = [[-1443.99, 1480.59]]
