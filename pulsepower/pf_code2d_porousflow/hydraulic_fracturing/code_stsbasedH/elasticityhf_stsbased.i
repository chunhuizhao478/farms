# =============================================================================
# Stress-Based Driving Force with PF-CZM Framework
# Based on Miehe & Mauthe (CMAME 2016) + Wu's PF-CZM (JMPS 2017)
# =============================================================================
#
# Key Features:
#   - Stress-based crack driving force using UNDEGRADED effective stress
#   - PF-CZM crack geometric function: α(d) = 2d - d²
#   - History maximum for irreversibility: H = max(H_old, D)
#
# Stress-Based Criterion (Miehe Eq. 56):
#   D = ζ * < Σ_{a=1}^{3} ( <σ̃_eff^a>_+ / σ_c )² - 1 >_+
#
# Where σ̃_eff is the UNDEGRADED effective stress (Miehe Eq. 95):
#   σ = g(d) * σ̃_eff - b*p*I  (total stress)
#   σ̃_eff = (σ + b*p*I) / g(d)  (recovered undegraded effective stress)
#
# Phase Field Evolution (Miehe Eq. 13):
#   η*ḋ = (1-d)*H - [d - l²Δd]
#
# =============================================================================

# Static energy references (from static solve)
fluid_elastic_energy_total_static = 1.973424e+04
solid_elastic_energy_total_static = 4.610351e+05
solid_dissipated_energy_total_static = 1.367156e+03
full_input_energy_static = 4.807693e+05
...
zeta = 1.0          # Dimensionless slope parameter (typically 1.0 - 10.0)

# =============================================================================
# PF-CZM Specific Parameters (Wu 2017)
# =============================================================================
# Tensile strength - should match sigma_c for consistency
ft = ${sigma_c}
...
# a1 parameter for PF-CZM degradation
...
[MultiApps]
  [fracture]
    ...
    input_files = fracturehf_stsbased.i
...
[Transfers]
  [from_d]
    ...
    source_variable = 'd'
  []
  # Transfer stress-based driving force H instead of psie_active
  [to_stress_driving_force]
    type = MultiAppCopyTransfer
    to_multi_app = 'fracture'
    variable = 'stress_driving_force mesh_size'
...
[AuxKernels]
  ...
  #   σ̃_eff = (σ + b*p*I) / g(d)
  #
  # For each principal direction a:
  #   σ̃_eff^a = (σ^a + b*p) / g(d)
  #
  # Then apply positive part <·>_+ for tension-driven fracture.
  # =============================================================================
...
[Materials]
  ...
  # =============================================================================
  # Elasticity Model with PF-CZM degradation and Porous Flow Coupling
  # =============================================================================
  [elasticity]
  ...
  # =============================================================================
  # Stress-Based Driving Force Implementation (Miehe Eq. 56, 95)
  # =============================================================================
  # The stress tensor from NDSmallDeformationIsotropicElasticity is DEGRADED:
  #   σ = g(d) * σ̃_eff - b*p*I  (Miehe Eq. 95)
  #
  # The driving force D (Miehe Eq. 56) requires UNDEGRADED effective stress:
  #   D = ζ * < Σ_a (<σ̃_eff^a>_+ / σ_c)² - 1 >_+
  #
  # We recover undegraded stress in the AuxKernel compute_stress_driving_force_D:
  #   σ̃_eff^a = (σ^a + b*p) / g(d)
  # =============================================================================
  ...
  # =============================================================================
  # Enhanced History Energy (kept for comparison, not used for driving force)
  # =============================================================================
...
[UserObjects]
  ...
  [./init_sol_components]
    ...
    mesh = elasticityhf_static_stsbased_exodus.e
  ...
  [./period0]
    ...
    disable_objects = '*/mass0 */inertia_x */inertia_y */vel_x */vel_y */accel_x */accel_y
                       ...
                       */pressure_inner_hole */pressure_inner_hole_fractures
                       MultiApps/fracture
                       Transfers/from_d Transfers/to_stress_driving_force
...
[Outputs]
  [./exodus]
    ...
    show = 'd vel_x vel_y vel_z pp psie_active_enhanced biot_modulus_aux biot_coefficient_aux porosity_aux max_principal_stress stress_driving_force_D stress_driving_force_H'
  [../]
  [checkpoint]
      type = Checkpoint
...
# =============================================================================
# Energy Calculation Postprocessors (same as PF-CZM)
# =============================================================================
[Postprocessors]
  [./dissipated_energy_dynamic]
    type = Receiver
