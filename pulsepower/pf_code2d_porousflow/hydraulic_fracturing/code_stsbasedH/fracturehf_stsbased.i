# =============================================================================
# Stress-Based Driving Force Fracture Sub-Application
...
[AuxVariables]
  [bounds_dummy]
  []
  # Stress-based driving force H transferred from main app
  [stress_driving_force]
    order = CONSTANT
    family = MONOMIAL
  []
  [Gc_var]
...
[Materials]
  ...
  # =============================================================================
  # Crack Geometric Function: alpha(d) = 2d - d^2 (PF-CZM)
  # This gives: c0 = pi, xi = 2, Du = pi*l/2 (finite support)
  # =============================================================================
  [crack_geometric]
  ...
  #
  # To get (1-d)*H as the driving term, we use:
  #   ψ_elastic = 0.5*(1-d)² * (Gc/(c0*l)) * H
  #
  # Then:
  #   ∂ψ_elastic/∂d = -(1-d) * (Gc/(c0*l)) * H
  #   -∂ψ_elastic/∂d = (1-d) * (Gc/(c0*l)) * H  ✓
  #
  # Combined with crack surface energy α(d)*Gc/(c0*l):
  #   ψ = α(d)*Gc/(c0*l) + 0.5*(1-d)² * (Gc/(c0*l)) * H
  # =============================================================================
  [psi]
