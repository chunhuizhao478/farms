...
[AuxVariables]
  ...
  [pulse_load_aux]
    ...
    family = MONOMIAL
  []
  # material biot coefficient exposed to a field for expressions
  [biot_coeff_aux]
    order = CONSTANT
    family = MONOMIAL
  []
...
[AuxKernels]
  ...
  [get_pulse_load_aux]
    ...
    execute_on = timestep_end
  []
  # get biot coefficient from material property
  [get_biot_coeff_aux]
...
[Kernels]
  ...
  [poro_x]
    type = ElkPorousFlowEffectiveStressCoupling
      # biot_coefficient = ${biot_coefficient} # ignored when use_damaged_biot=true
...
[Materials]
  ...
  # M(d) = ( (alpha(d) - phi) / Ks + phi / Kf )^-1 = ( (1 - alpha(d))(alpha(d)-phi) / K + phi / Kf )^-1
  [biot_modulus]
    ...
    outputs = exodus
  []
  # Damaged Biot coefficient material property
  # alpha(d) = 1 - K(d) / Ks
  [biot_coefficient]
    type = ElkPorousFlowDamagedBiotCoefficient
    grain_bulk_modulus = ${fparse 1.0/(1.0-initial_biot_coefficient)*K} #K_s
  []
  ##----------------------------------------------------------##
...
[AuxKernels]
  ...
  coupled_variables = 'elastic_strain_00 elastic_strain_11 elastic_strain_22 pp biot_coeff_aux'
  expression = "0.5 * biot_coeff_aux * -pp * (elastic_strain_00+elastic_strain_11+elastic_strain_22)"
  []
[]
...
[AuxKernels]
  [fluid_incremental_elastic_energy_per_vol]
      ...
      coupled_variables = 'strain_increment_00 strain_increment_11 strain_increment_22 pp biot_coeff_aux'
      expression = "biot_coeff_aux * -pp * (strain_increment_00+strain_increment_11+strain_increment_22)"
  []
[]
