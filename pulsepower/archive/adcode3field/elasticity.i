...
[AuxKernels]
  ...
  #     execute_on = timestep_end
  # []
  #calculate a^f_{n+1} and v^f_{n+1}
  [af_x]
      type = NewmarkAccelAux
      variable = af_x
      displacement = wf_x
...
[Kernels]
    ...
    #     gamma = 0.5
    # []
    #pore fluid inertia force coupling (rho^f a^f)
    [porefluidinertia_x]
    ...
    #     multiply_biot_coefficient = true
    # []
    #dynamic darcy flow (rho^f a^s + rho^f tau_t / phi * a^f + mu_f / kappa * w)
    [dynamicdarcyflow_x]
        type = ElkADDynamicDarcyFlowSmearedCracking
        variable = wf_x
        skeletondisp = disp_x
        skeletonvel = vel_x
    ...
    #     component = 2
    # []
    #poro mechanical coupling (-p div w)
    [poromechanic_wx]
        type = ElkADPoroMechanicsCoupling
        variable = wf_x
        porepressure = p
    ...
    #     multiply_biot_coefficient = false
    # []
    #pressure rate (p dot dp)
    [pressure_rate]
        type = ElkADPressureRate
        variable = p
    []
    #mass conservation (alpha nabla v^s dp)
...
[Materials]
  ...
  [porodynamics] # take material property: effective_perm
    ...
    output_properties = 'biot_modulus'
    outputs = exodus
  []
[]
...
[UserObjects]
  [./init_sol_components]
    ...
    mesh = ./static_solve_elasticity_out.e
