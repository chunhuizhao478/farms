# Permeability enhancement: legacy Darcy-Poiseuille formulation (pre-Heider).
# Aperture w = d * wc, fracture perm K_frac = (w^2/12) * I (isotropic),
# total K = k0*I + d^perm_exponent * (K_frac - k0*I). Mirror of
# permeability_formula/ — only the permeability model differs.
#
...
##legacy Darcy-Poiseuille permeability model:
##  aperture w = d * wc,   K_frac = (w^2 / 12) * I   (isotropic)
##  total K = k0*I + d^perm_exponent * (K_frac - k0*I)
wc = '${fparse Gc_const / ft}' # ultimate crack width, m
perm_exponent = 10 # damage localization exponent
#----------------------------------------------------#
...
[Materials]
  ...
  [elasticity]
    ...
    ##-----darcy_poiseuille_permeability_model (legacy w = d*wc)-----##
    permeability_model = darcy_poiseuille
    intrinsic_permeability = ${intrinsic_permeability}
    wc = ${wc}                                # ultimate crack width
    perm_exponent = ${perm_exponent}          # exponent in K = k0*I + d^p*(kf - k0*I)
    ##---------------------------------------------##
  []
  [stress]
