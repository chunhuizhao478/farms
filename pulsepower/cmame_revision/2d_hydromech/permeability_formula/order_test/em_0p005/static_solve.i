...
[Mesh]
  ...
  [Variables]
    # P2-P1 (Taylor-Hood): disp SECOND, pp FIRST. Required because the
    # 2nd-order mesh rejects P1 displacements (MOOSE: "mesh has SECOND
    # order elements, so all displacement variables must be SECOND order").
    [disp_x]
        order = SECOND
        family = LAGRANGE
        scaling = 1e-6
  ...
  [AuxVariables]
    # Reaction-force auxvars must match disp order (save_in_disp_x/_y
    # compatibility).
    [fx]
        order = SECOND
        family = LAGRANGE
 ...
 []
  ...
  [Executioner]
    ...
    # Direct LU (superlu_dist) handles the P2-P1 Taylor-Hood saddle structure
    # reliably. The iterative gmres+boomeramg used by the parent P1-P1 static
    # stalls on the P2-P1 Jacobian and is why the earlier attempts failed.
