...
[Outputs]
  # Enable exodus so the Weibull-sampled Gc field (Gc_var aux variable)
  # can be inspected in ParaView. Gc_var is set once by RandomIC at INITIAL
  # and never changes, so a single step is sufficient — but we keep default
  # cadence for consistency with the elasticity-side exodus output.
  exodus = true
...
[Distributions]
  ...
  [weibull]
    type = Weibull
    shape = 15.0 #k — parametric case: shape = 15
