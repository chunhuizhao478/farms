# =============================================================================
# PF-CZM Three-Field Poroelastodynamics with Phase-Field Damage (2D)
...
#   - p: pore pressure
#
# Key PF-CZM differences from AT1:
#   - Crack geometric function: alpha(d) = 2d - d^2 (vs d for AT1)
#   - Normalization constant: c0 = pi (vs 8/3 for AT1)
#   - Degradation function: Rational form with explicit failure strength
#   - Tensile strength ft is an explicit material parameter
#
# Coupling: MultiApp phase-field damage with staggered scheme
# =============================================================================

# Static energy values from static solve
fluid_elastic_energy_total_static = 7.459564e-05
solid_elastic_energy_total_static = 5.415555e-03
full_input_energy_static = 5.494172e-03
...
# =============================================================================
# Tensile strength - derived from AT1 formula for consistency
# For AT1: ft = sqrt(3/8 * E * Gc / l)
...
# a1 parameter: a1 = Gc/(psic*c0*l/xi) = (4/pi) * lch / l
# where lch = E*Gc/ft^2 is Irwin's characteristic length
a1_val = '${fparse 4.0 / c0_val * E * Gc_const / (ft * ft * l)}'
...
intrinsic_permeability = 5e-15  # m^2
tortosity = 1.2

# Darcy-Poiseuille permeability model
wc = ${fparse Gc_const / ft}  # Ultimate crack width [m]
perm_exponent = 10
...
[AuxVariables]
  ...
  [strain_22]
    ...
    family = MONOMIAL
  []
  # Strain increment components for fluid dissipation energy calculation
  [strain_increment_00]
    order = CONSTANT
    family = MONOMIAL
  []
  ...
  [fdampy]
    ...
    family = LAGRANGE
  []
  # Fluid boundary power for energy tracking
...
[AuxKernels]
  ...
  [extract_strain_22]
    ...
    execute_on = 'TIMESTEP_END'
  []
  # Strain increment components for fluid dissipation energy
  [extract_strain_increment_00]
    type = ADRankTwoAux
...
[BCs]
  ...
  [./Pressure]
    ...
    []
  []
  # Fix corner point - only solid displacement to prevent rigid body motion
  # Note: Do NOT fix fluid displacement (wf) or pressure (p) at corner points
  # as this creates artificial stress concentrations and pressure oscillations
  [./fix_cptr1_x]
    type = DirichletBC
    variable = disp_x
    boundary = corner_ptr
...
[Materials]
  ...
  # Note: ADSmallDeformationIsotropicElasticityPF computes g internally for PF_CZM
  # =============================================================================
  [pfczm_params]
    type = ADGenericConstantMaterial
    prop_names = 'a1_mat a2_mat a3_mat p_mat'
    prop_values = '${a1_val} ${a2} ${a3} ${p_deg}'
  []

  # =============================================================================
  # AD Phase-field elasticity with PF-CZM model and damage-dependent permeability
  # =============================================================================
  [elasticity]
  ...
  # =============================================================================
  # AD Three-field enhanced history energy with pressure
  # =============================================================================
  [history_energy_enhanced]
    ...
    # Use two-field formulation for testing
    use_two_field_formulation = true
  []
...
[AuxKernels]
  [fluid_incremental_elastic_energy_per_vol]
    ...
    coupled_variables = 'strain_increment_00 strain_increment_11 strain_increment_22 p biot_coefficient_aux'
    expression = "biot_coefficient_aux * -p * (strain_increment_00+strain_increment_11+strain_increment_22)"
  []
[]
