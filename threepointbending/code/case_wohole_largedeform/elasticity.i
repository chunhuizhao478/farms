...
l = 1e-4 # N * h, N: number of elements, h: element size
...
[AuxVariables]
  [d]
  []
  # Hencky (logarithmic) strain component
  [strain_00]
    order = CONSTANT
    family = MONOMIAL
    block = 1
...
[AuxKernels]
  # Output Hencky strain xx component
  [strain_00_aux]
    type = ADRankTwoAux
    variable = strain_00
...
[Materials]
  ...
  []
  # Block 1: Phase field region with large deformation
  [defgrad_phasefield]
  ...
  [stress_phasefield]
    ...
    elasticity_model = hencky_phasefield
    output_properties = 'stress'
    outputs = exodus
    block = 1
  []
  # Block 2: Elastic regions with large deformation (no phase field)
  [defgrad_elastic]
    type = ComputeDeformationGradient
    block = 2
  []
  [no_degradation_elastic]
    type = NoDegradation
    property_name = g_elastic
    expression = 1
    phase_field = d
    block = 2
  []
  [hencky_elastic]
    ...
    shear_modulus = G
    phase_field = d
    degradation_function = g_elastic
    decomposition = NONE
    block = 2
  []
  [stress_elastic]
