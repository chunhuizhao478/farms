...
InputParameters
VariableBoundsBase::validParams()
{
...
VariableBoundsBase::VariableBoundsBase(const InputParameters & parameters)
...
Real
VariableBoundsBase::computeValue()
{
  ...
  {
    Real bound;
    // Use old value after the first time step
    if (_t_step > 1)
      bound = _fe_var->getNodalValueOld(*_current_node); //set equal to the old value
    else
      bound = getBound(); //get the variable bound value
...
dof_id_type
VariableBoundsBase::getDoFIndex() const
{
  if (isNodal())
  {
