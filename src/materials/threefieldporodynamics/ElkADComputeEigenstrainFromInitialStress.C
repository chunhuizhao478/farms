...
#include "ElkADComputeEigenstrainFromInitialStress.h"
#include "RankTwoTensor.h"
#include "Function.h"
#include "Conversion.h" // for stringify

registerMooseObject("farmsApp", ElkADComputeEigenstrainFromInitialStress);

InputParameters
ElkADComputeEigenstrainFromInitialStress::validParams()
{
...
ElkADComputeEigenstrainFromInitialStress::ElkADComputeEigenstrainFromInitialStress(
                               ...
                               : std::vector<const VariableValue *>())
{
  ...
  if (num != LIBMESH_DIM * LIBMESH_DIM)
    paramError(
        "initial_stress",
        "ElkADComputeEigenstrainFromInitialStress: " + Moose::stringify(LIBMESH_DIM * LIBMESH_DIM) +
  ...
  if (_ini_aux_provided)
  {
    ...
    if (aux_size != LIBMESH_DIM * LIBMESH_DIM)
      paramError("initial_stress_aux",
                 "ElkADComputeEigenstrainFromInitialStress: If you supply initial_stress_aux, " +
...
void
ElkADComputeEigenstrainFromInitialStress::computeQpEigenstrain()
{
  if (_t_step == 1)
  {
