...
#include "ElkPorousFlowHistoryEnergyEnhanced.h"

registerMooseObject("farmsApp", ElkPorousFlowHistoryEnergyEnhanced);

InputParameters
ElkPorousFlowHistoryEnergyEnhanced::validParams()
{
  InputParameters params = Material::validParams();
  params.addClassDescription(
      "Computes the enhanced history energy H+ that includes pressure-dependent terms "
      "for poroelastic coupling in phase field fracture. Implements equation (A.12) "
      "from the CMAME paper Appendix A.");

  params.addRequiredParam<MaterialPropertyName>(
      "psie_active",
      "Name of the positive elastic energy density material property");
...
ElkPorousFlowHistoryEnergyEnhanced::ElkPorousFlowHistoryEnergyEnhanced(
...
void
ElkPorousFlowHistoryEnergyEnhanced::computeQpProperties()
{
  // Compute the pressure-dependent coefficient
  // coeff = (φ_o - 1)*(1/K_f - 1/K_s) - K/K_s²
  const Real coeff = computePressureCoefficient();

  // Compute the enhanced history energy before applying Macaulay bracket
  // H_current = 2*ψ_e^{e+} + p²*coeff
  const Real H_current = 2.0 * _psie_active[_qp] + _pp[_qp] * _pp[_qp] * coeff;

  // Apply Macaulay bracket: <x>^+ = max(0, x)
  const Real H_current_positive = std::max(0.0, H_current);
...
Real
ElkPorousFlowHistoryEnergyEnhanced::computePressureCoefficient() const
{
