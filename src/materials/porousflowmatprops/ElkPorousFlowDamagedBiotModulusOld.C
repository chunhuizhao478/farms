...
#include "ElkPorousFlowDamagedBiotModulusOld.h"

registerMooseObject("farmsApp", ElkPorousFlowDamagedBiotModulusOld);

InputParameters
ElkPorousFlowDamagedBiotModulusOld::validParams()
{
...
ElkPorousFlowDamagedBiotModulusOld::ElkPorousFlowDamagedBiotModulusOld(const InputParameters & parameters)
...
void
ElkPorousFlowDamagedBiotModulusOld::initQpStatefulProperties()
{
...
void
ElkPorousFlowDamagedBiotModulusOld::computeQpProperties()
{
  const Real alpha = _use_damaged_biot ? (*_biot_coefficient_matprop)[_qp] : _biot_coefficient_const;
  _biot_modulus[_qp] = 1.0 / ((1.0 - alpha) * (alpha - _porosity[_qp]) *
