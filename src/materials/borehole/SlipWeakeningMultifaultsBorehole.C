...
#include "SlipWeakeningMultifaultsBorehole.h"
#include "InterfaceKernel.h"

registerMooseObject("farmsApp", SlipWeakeningMultifaultsBorehole);

InputParameters
SlipWeakeningMultifaultsBorehole::validParams()
{
...
SlipWeakeningMultifaultsBorehole::SlipWeakeningMultifaultsBorehole(const InputParameters & parameters)
...
void
SlipWeakeningMultifaultsBorehole::computeInterfaceTractionAndDerivatives()
{   
   ...
   // add limits -2MPa
    ...
    //penetration, add large normal traction value
    if (displacement_jump(0) < 0){
      std::cout<<displacement_jump(0)<<std::endl;
      T2 = 1e16;
    }
