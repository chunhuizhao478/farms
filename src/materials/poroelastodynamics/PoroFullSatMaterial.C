...
registerMooseObject("farmsApp", PoroFullSatMaterial);
...
PoroFullSatMaterial::validParams()
{
  ...
  params.addRequiredRangeCheckedParam<Real>(
      "solid_bulk_modulus",
      "solid_bulk_modulus>=0",
      "The solid bulk modulus (the reciprocal of the solid bulk modulus)");
  params.addRequiredRangeCheckedParam<Real>(
      "fluid_bulk_modulus",
      "fluid_bulk_modulus>=0",
      "The fluid bulk modulus (the reciprocal of the fluid bulk modulus)");
...
PoroFullSatMaterial::PoroFullSatMaterial(const InputParameters & parameters)
  : DerivativeMaterialInterface<Material>(parameters),
    ...
    _Kf(getParam<Real>("fluid_bulk_modulus")),
...
PoroFullSatMaterial::computeQpProperties()
{
  ...
  if (_constant_porosity)
  {
    ...
    _one_over_biot_modulus[_qp] =
        // ( _porosity[_qp] * _Ks - ( _alpha - _porosity[_qp] ) * _Kf) / (_Ks*_Kf);
        (1 - _alpha) * (_alpha - _porosity[_qp]) * 1/_Ks + _porosity[_qp] * 1/_Kf;
  ...
  {
    Real one_over_K = 1/_Ks;
    Real one_over_Kf = 1/_Kf;
    _porosity[_qp] =
        _alpha + (_phi0 - _alpha) *
                     std::exp(-(1 - _alpha) * one_over_K * _porepressure[_qp] - _vol_strain[_qp]);
    _dporosity_dP[_qp] =
        (_phi0 - _alpha) * (_alpha - 1) * one_over_K *
        std::exp(-(1 - _alpha) * one_over_K * _porepressure[_qp] - _vol_strain[_qp]);
    _dporosity_dep[_qp] =
        -(_phi0 - _alpha) *
        std::exp(-(1 - _alpha) * one_over_K * _porepressure[_qp] - _vol_strain[_qp]);
    ...
    _one_over_biot_modulus[_qp] =
        (1 - _alpha) * (_alpha - _porosity[_qp]) * one_over_K + _porosity[_qp] * one_over_Kf;
    _done_over_biot_modulus_dP[_qp] =
        -(1 - _alpha) * _dporosity_dP[_qp] * one_over_K + _dporosity_dP[_qp] * one_over_Kf;
    _done_over_biot_modulus_dep[_qp] =
        -(1 - _alpha) * _dporosity_dep[_qp] * one_over_K + _dporosity_dep[_qp] * one_over_Kf;
  }
}