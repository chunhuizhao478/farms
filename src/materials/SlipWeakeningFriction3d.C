...
registerMooseObject("farmsApp", SlipWeakeningFriction3d);
...
double computeMusDistribution3D(Real x_coord, Real z_coord)
{
  double mu_s = 0.0;
...
//Problem-Specific: TPV205-3D
double computeT1oDistribution3D(Real x_coord, Real z_coord)
{
  double T1_o = 0.0;
...
{   
  ...
  Real shear_x = stress_y(0);
  //Real normal = stress_y(1); 
  Real shear_z = stress_z(1);
  ...
  //Compute node mass
  Real M = _density[_qp] * area * area * area /2 ; 

  //Compute mu_s for current qp
  mu_s = computeMusDistribution3D(x_coord, z_coord);

  //Compute T1_o for current qp
  T1_o = computeT1oDistribution3D(x_coord, z_coord);

  //Compute sticking stress
  Real T1 = (1/_dt)*M*displacement_jump_rate(1)/(2*area*area) + shear_x + T1_o;
  Real T3 = -(1/_dt)*M*displacement_jump_rate(2)/(2*area*area) + shear_z + 0.0;
  Real T2  = - T2_o; 

  //Compute friction strength
  if (std::norm(_interface_displacement_jump[_qp]) < Dc)
  {
    tau_f = (mu_s - (mu_s - mu_d)*std::norm(_interface_displacement_jump[_qp])/Dc)*(-T2); // square for shear component
  }
  else
  {
    tau_f = mu_d * (-T2);
