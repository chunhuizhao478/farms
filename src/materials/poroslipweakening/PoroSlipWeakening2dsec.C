...
#include "PoroSlipWeakening2dsec.h"
#include "InterfaceKernel.h"
#include "FEProblem.h"

registerMooseObject("farmsApp", PoroSlipWeakening2dsec);

InputParameters
PoroSlipWeakening2dsec::validParams()
{
...
PoroSlipWeakening2dsec::PoroSlipWeakening2dsec(const InputParameters & parameters)
...
double porocomputeMusDistribution2DReactv8(Real x_coord)
{
  double mu_s = 0.0;
...
//Problem-Specific: TPV205-2D
double porocomputeT1oDistribution2DReactv8(Real x_coord)
{
  double T1_o = 0.0;
...
void
PoroSlipWeakening2dsec::computeInterfaceTractionAndDerivatives()
{   
 ...
 //Parameter initialization
  ...
  //Compute pressure on plus/minus faces in local coordinate
   ...
   mu_s = porocomputeMusDistribution2DReactv8(x_coord);

   //Compute T1_o for current qp
   T1_o = porocomputeT1oDistribution2DReactv8(x_coord);

   //Compute sticking stress
    // Real T1 =  -(1/_dt)*M*displacement_jump_rate(1)/(2*area) + shear + T1_o;
    //  Real T2 = - T2_o + p * (-alpha)  + (1/_dt)*Mf*(fluid_vel_jump(0)+(1/_dt)*fluid_disp_jump(0))/(2*area)  ;
  ...
  // Define pressure based on fault permeability condition
    ...
    Real T2 =  (1/_dt)*M*(displacement_jump_rate(0)+(1/_dt)*displacement_jump(0))/(2*area) 
              ...
              +( (R_plus_local_y + R_plus_damp_local_y - R_minus_local_y - R_minus_damp_local_y) / ( 2*area) 
              - alpha * (p) ) - T2_o;
  ...
  //Compute sticking stress derivatives
   ...
   {
     tau_f = (mu_s - (mu_s - mu_d)*std::abs(displacement_jump(1))/Dc)*(-T2 - p); // square for shear component

     dtau_f_ddipsx = -(mu_s - mu_d)/Dc*(-T2 - p)*(displacement_jump(1))/(std::abs(displacement_jump(1)));
