...
#include "ComputeDamageBreakageStress3Ddebug.h"
#include "NestedSolve.h"
#include "FEProblem.h"

registerMooseObject("farmsApp", ComputeDamageBreakageStress3Ddebug);

InputParameters
ComputeDamageBreakageStress3Ddebug::validParams()
{ 
...
ComputeDamageBreakageStress3Ddebug::ComputeDamageBreakageStress3Ddebug(const InputParameters & parameters)
...
void
ComputeDamageBreakageStress3Ddebug::initialSetup()
{
...
void
ComputeDamageBreakageStress3Ddebug::initQpStatefulProperties()
{
...
{ 
  ...
  if (_t == 0)
  {
    ...
    */
    ...
    RankTwoTensor eps_e = _eps_e_old[_qp] + _mechanical_strain[_qp] - _mechanical_strain_old[_qp] - eps_p;

    const Real epsilon = -1e-16;
    Real I1 = epsilon + eps_e(0,0) + eps_e(1,1) + eps_e(2,2);
    Real I2 = epsilon * epsilon + eps_e(0,0) * eps_e(0,0) + eps_e(1,1) * eps_e(1,1) + eps_e(2,2) * eps_e(2,2) + 2 * eps_e(0,1) * eps_e(0,1) + 2 * eps_e(0,2) * eps_e(0,2) + 2 * eps_e(1,2) * eps_e(1,2);
