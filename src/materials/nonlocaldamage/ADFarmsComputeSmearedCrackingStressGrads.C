...
registerMooseObject("farmsApp", ADFarmsComputeSmearedCrackingStressGrads);
...
ADFarmsComputeSmearedCrackingStressGrads::ADFarmsComputeSmearedCrackingStressGrads(const InputParameters & parameters)
  : ADComputeMultipleInelasticStress(parameters),
    ...
    _eqstrain_nonlocal(adCoupledValue("eqstrain_nonlocal")),
...
ADFarmsComputeSmearedCrackingStressGrads::computeQpStress()
{
  ...
  ADReal kappa = std::max(_kappa_old[_qp], _eqstrain_nonlocal[_qp]);
  _kappa[_qp] = kappa;
  ...
  if (kappa > eps0)
  {
    ADReal arg1 = std::exp(-_paramB * (kappa - eps0));
    ADReal arg2 = 1 - _paramA + _paramA * arg1;
