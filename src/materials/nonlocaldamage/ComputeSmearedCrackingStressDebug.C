...
#include "ComputeSmearedCrackingStressDebug.h"
#include "ElasticityTensorTools.h"
#include "StressUpdateBase.h"
#include "Conversion.h"

registerMooseObject("farmsApp", ComputeSmearedCrackingStressDebug);

InputParameters
ComputeSmearedCrackingStressDebug::validParams()
{
...
ComputeSmearedCrackingStressDebug::ComputeSmearedCrackingStressDebug(const InputParameters & parameters)
...
void
ComputeSmearedCrackingStressDebug::initQpStatefulProperties()
{
...
void
ComputeSmearedCrackingStressDebug::initialSetup()
{
  ComputeMultipleInelasticStress::initialSetup();

  if (!hasGuaranteedMaterialProperty(_elasticity_tensor_name, Guarantee::ISOTROPIC))
    mooseError("ComputeSmearedCrackingStressDebug requires that the elasticity tensor be "
...
void
ComputeSmearedCrackingStressDebug::computeQpStress()
{
...
void
ComputeSmearedCrackingStressDebug::updateLocalElasticityTensor()
{
...
void
ComputeSmearedCrackingStressDebug::updateCrackingStateAndStress()
{
  const Real youngs_modulus =
      ElasticityTensorTools::getIsotropicYoungsModulus(_elasticity_tensor[_qp]);
  ...
  if (cracking_stress > 0)
  {
    ...
    Real eqstrain_local = std::sqrt(strain_dir0_positive * strain_dir0_positive +
    ...
    //     _crack_max_strain[_qp](i) = strain_in_crack_dir(i);
      ...
      if (eqstrain > _crack_max_strain[_qp](i))
...
void
ComputeSmearedCrackingStressDebug::computeCrackStrainAndOrientation(
    RealVectorValue & strain_in_crack_dir)
{
...
unsigned int
ComputeSmearedCrackingStressDebug::getNumKnownCrackDirs() const
{
...
void
ComputeSmearedCrackingStressDebug::updateStressTensorForCracking(RankTwoTensor & tensor,
...
bool
ComputeSmearedCrackingStressDebug::previouslyCracked()
{
  for (unsigned int i = 0; i < 3; ++i)
    if (_crack_damage_old[_qp](i) > 0.0)
      return true;
