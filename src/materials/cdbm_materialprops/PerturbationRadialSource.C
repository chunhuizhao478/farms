...
#include "PerturbationRadialSource.h"
 ...
 */
registerMooseObject("farmsApp", PerturbationRadialSource);

InputParameters
PerturbationRadialSource::validParams()
{
...
PerturbationRadialSource::PerturbationRadialSource(const InputParameters & parameters)
  : Material(parameters),
...
void
PerturbationRadialSource::initQpStatefulProperties()
{
...
void
PerturbationRadialSource::computeQpProperties()
{
  ...
  if (ycoord >= _nucl_center[1] - _thickness / 2.0 && ycoord <= _nucl_center[1] + _thickness / 2.0)
  {
    if (_perturbation_type == "damage")
    {
      ...
      _damage_perturbation[_qp] = source;
      _breakage_perturbation[_qp] = 0.0;
      _shear_stress_perturbation[_qp] = 0.0;
    }
    else if (_perturbation_type == "shear_stress")
    {
      _damage_perturbation[_qp] = 0.0;
      _breakage_perturbation[_qp] = 0.0;
      _shear_stress_perturbation[_qp] = source;
    }
    else if (_perturbation_type == "breakage")
    {
      _damage_perturbation[_qp] = 0.0;
      _breakage_perturbation[_qp] = source;
      _shear_stress_perturbation[_qp] = 0.0;
    }
    else
    {
