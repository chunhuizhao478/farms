...
#include "FarmsComputeLagrangianStrain.h"

template <class G>
InputParameters
FarmsComputeLagrangianStrainBase<G>::baseParams()
{
...
template <class G>
FarmsComputeLagrangianStrainBase<G>::FarmsComputeLagrangianStrainBase(const InputParameters & parameters)
  : Material(parameters),
    ...
    _rotation_increment(declareProperty<RankTwoTensor>(_base_name + "rotation_increment"))
{
  ...
  if (_homogenization_gradient_names.size() > 1)
    mooseError("FarmsComputeLagrangianStrainBase cannot accommodate more than one "
...
template <class G>
void
FarmsComputeLagrangianStrainBase<G>::initQpStatefulProperties()
{
...
template <class G>
void
FarmsComputeLagrangianStrainBase<G>::computeProperties()
{
...
template <class G>
void
FarmsComputeLagrangianStrainBase<G>::computeQpProperties()
{
...
template <class G>
void
FarmsComputeLagrangianStrainBase<G>::computeQpIncrementalStrains(const RankTwoTensor & dL)
{
  ...
  // add in the eigenstrain increment
  for (auto i : make_range(_eigenstrain_names.size()))
    _mechanical_strain[_qp] -= (*_eigenstrains[i])[_qp];
...
template <class G>
void
FarmsComputeLagrangianStrainBase<G>::subtractQpEigenstrainIncrement(RankTwoTensor & strain)
{
  for (auto i : make_range(_eigenstrain_names.size()))
...
template <class G>
void
FarmsComputeLagrangianStrainBase<G>::computeQpUnstabilizedDeformationGradient()
{
...
template <class G>
void
FarmsComputeLagrangianStrainBase<G>::computeDeformationGradient()
{
