...
ComputeLagrangianDamageBreakageStressPK2Debug::computeQpPK1Stress()
{
  ...
  // //Since Fp is updated explicitly using variables from the previous time step, the derivative dFp/dF is zero.
    ...
    for (unsigned int j = 0; j < 3; j++){
      for (unsigned int k = 0; k < 3; k++){
        for (unsigned int l = 0; l < 3; l++){
          ...
          for (unsigned int m = 0; m < 3; m++){
            for (unsigned int n = 0; n < 3; n++){
              accum += dFedF_tensor[i][m][k][l] * _S[_qp](m, n) * Fpinv(j, n);
            }
          }
          // Term 2
          for (unsigned int m = 0; m < 3; m++){
            for (unsigned int n = 0; n < 3; n++){
              for (unsigned int p = 0; p < 3; p++){
                for (unsigned int q = 0; q < 3; q++){
                  accum += _Fe[_qp](i, m) * _C[_qp](m, n, p, q) * dEdF_tensor[p][q][k][l] * Fpinv(j, n);
                }
              }
            }
          }
          ...
          for (unsigned int m = 0; m < 3; m++){
            for (unsigned int n = 0; n < 3; n++){
              accum += _Fe[_qp](i, m) * _S[_qp](m, n) * dFpmdF_tensor[j][n][k][l];
            }
          }
