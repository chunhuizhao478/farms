...
#include "ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit.h"


registerMooseObject("farmsApp", ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit);

InputParameters
ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit::validParams()
{
...
ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit::ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit(const InputParameters & parameters)
  ...
  _pore_pressure_older(coupledValueOlder("porepressure")),
  ...
  _eta_older(getMaterialPropertyOlderByName<Real>(_base_name + "plastic_volume_change")),
  ...
  _Fp_older(getMaterialPropertyOlderByName<RankTwoTensor>(_base_name + "plastic_deformation_gradient")),
  ...
  //---------------------------------------------------------------------------------------------//
  // Add these lines to the Explicit central difference of some quantity
  _Fp_dot_old(getMaterialPropertyOldByName<RankTwoTensor>(_base_name + "cdbm_plastic_deformation_gradient_rate")),
  _F_dot_old(getMaterialPropertyOldByName<RankTwoTensor>(_base_name + "cdbm_deformation_gradient_rate")),
  _Fp_dot_older(getMaterialPropertyOlderByName<RankTwoTensor>(_base_name + "cdbm_plastic_deformation_gradient_rate")),
  _F_dot_older(getMaterialPropertyOlderByName<RankTwoTensor>(_base_name + "cdbm_deformation_gradient_rate")),
  //---------------------------------------------------------------------------------------------//
...
//
void
ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit::initQpStatefulProperties()
{
...
void
ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit::computeQpPK1Stress()
{
  ...
  // --------------------------------------------------------------------------
  // Central Difference: Compute dDpdF_tensor(i,j,k,l) = ∂Dp_ij / ∂F_kl
  // Using: Dp = (Fp^{n+1} - Fp^{n-1}) / (2*dt) * Fp^{-1}
  // --------------------------------------------------------------------------

  // Map time levels to MOOSE material properties (use different variable names)
  RankTwoTensor Fp_current = _Fp[_qp];        // Current (being computed)
  RankTwoTensor Fp_previous = _Fp_old[_qp];   // Previous time step
  RankTwoTensor Fp_twosteps = _Fp_older[_qp]; // Two time steps ago
  RankTwoTensor Fpinv_cd = Fp_previous.inverse();  // Inverse at time n for central difference

  // Compute derivative tensors using existing method
  Real dFpdF_current[3][3][3][3];   // ∂Fp^{current}/∂F
  Real dFpdF_previous[3][3][3][3];  // ∂Fp^{n}/∂F
  Real dFpdF_twosteps[3][3][3][3];  // ∂Fp^{n-1}/∂F

  // Current derivatives (already computed in your file - reuse the existing dFpdF_tensor)
  for (unsigned int i = 0; i < 3; i++){
    for (unsigned int j = 0; j < 3; j++){
      for (unsigned int k = 0; k < 3; k++){
        for (unsigned int l = 0; l < 3; l++){
          // Use existing calculation for current time
          dFpdF_current[i][j][k][l] = dFpdF_tensor[i][j][k][l];
          
          // Previous time step (n)
          if (_dt == 0.0 || _Fp_dot_old[_qp](i,j) == 0.0 || _F_dot_old[_qp](k,l) == 0.0)
            dFpdF_previous[i][j][k][l] = 0.0;
          else
            dFpdF_previous[i][j][k][l] = _Fp_dot_old[_qp](i,j) / _F_dot_old[_qp](k,l);
          
          // Two time steps ago (n-1)
          if (_dt == 0.0 || _Fp_dot_older[_qp](i,j) == 0.0 || _F_dot_older[_qp](k,l) == 0.0)
            dFpdF_twosteps[i][j][k][l] = 0.0;
          else
            dFpdF_twosteps[i][j][k][l] = _Fp_dot_older[_qp](i,j) / _F_dot_older[_qp](k,l);
        }
      }
    }
  }

  // Zero target tensor first
  for (unsigned int i=0;i<3;i++)
    for (unsigned int j=0;j<3;j++)
      for (unsigned int k=0;k<3;k++)
        for (unsigned int l=0;l<3;l++)
          _dDpdF[_qp](i,j,k,l) = 0.0;

  // Factor for central difference
  const Real inv2dt_cd = 0.5 / _dt;

  // Precompute deltaFp for central difference
  RankTwoTensor deltaFp_cd; deltaFp_cd.zero();
  for (unsigned int i=0;i<3;i++)
    for (unsigned int j=0;j<3;j++)
      deltaFp_cd(i,j) = Fp_current(i,j) - Fp_twosteps(i,j);
    ...
    for (unsigned int j = 0; j < 3; ++j)
    {
      for (unsigned int k = 0; k < 3; ++k)
      {
        for (unsigned int l = 0; l < 3; ++l)
        {
          ...
          // Term A: (1/(2 dt)) * ∂Fp^{n+1}_{iα}/∂F_{kl} * Fpinv(α,j)
          for (unsigned int m = 0; m < 3; ++m)
          {
            accum += inv2dt_cd * dFpdF_current[i][m][k][l] * Fpinv_cd(m, j);
          }
          
          // Term B: -(1/(2 dt)) * ∂Fp^{n-1}_{iα}/∂F_{kl} * Fpinv(α,j)
          for (unsigned int m = 0; m < 3; ++m)
          {
            accum -= inv2dt_cd * dFpdF_twosteps[i][m][k][l] * Fpinv_cd(m, j);
          }
          
          // Term C: (1/(2 dt)) * (Fp^{n+1}_{iα} - Fp^{n-1}_{iα}) * ∂(Fp^{-1}_{α j})/∂F_{kl}
          // Use: ∂Fp^{-1}_{α j}/∂F_{kl} = - Fpinv(α,p) * (∂Fp_{p q}/∂F_{kl}) * Fpinv(q,j)
          for (unsigned int alpha = 0; alpha < 3; ++alpha)
          {
            for (unsigned int p = 0; p < 3; ++p)
            {
              for (unsigned int q = 0; q < 3; ++q)
              {
                accum += - inv2dt_cd * deltaFp_cd(i, alpha) * Fpinv_cd(alpha, p) * dFpdF_previous[p][q][k][l] * Fpinv_cd(q, j);
              }
            }
          }
          
          _dDpdF[_qp](i, j, k, l) = accum;
        }
      }
    }
  }
  ...
  // --------------------------------------------------------------------------
  // Central Difference: Compute dDpdp_tensor(i,j) = ∂Dp_ij / ∂p
  // Using: Dp = (Fp^{n+1} - Fp^{n-1}) / (2*dt) * Fp^{-1}
  // ⇒ ∂Dp/∂p = (1/(2*dt)) * [∂Fp^{n+1}/∂p * Fp^{-1} + (Fp^{n+1} - Fp^{n-1}) * ∂(Fp^{-1})/∂p]
  // Note: ∂Fp^{n-1}/∂p = 0 (already known from previous time step)
  // --------------------------------------------------------------------------

  // Compute dFp^{n+1}/dp (current plastic deformation gradient derivative w.r.t. pressure)
  RankTwoTensor dFp_current_dp; 
  dFp_current_dp.zero();
  Real dp_cd = _pore_pressure[_qp] - _pore_pressure_old[_qp];

  if (_dt != 0.0 && std::abs(dp_cd) > 1e-12){
    for (unsigned int i = 0; i < 3; ++i){
      for (unsigned int j = 0; j < 3; ++j){
        dFp_current_dp(i,j) = _Fp_dot[_qp](i,j) / dp_cd;  // Current Fp derivative
      }
    }
  }

  // Compute dFp^{n}/dp (for inverse derivative calculation)
  RankTwoTensor dFp_previous_dp;
  dFp_previous_dp.zero();
  Real dp_old_cd = _pore_pressure_old[_qp] - _pore_pressure_older[_qp];

  if (_dt != 0.0 && std::abs(dp_old_cd) > 1e-12){
    for (unsigned int i = 0; i < 3; ++i){
      for (unsigned int j = 0; j < 3; ++j){
        dFp_previous_dp(i,j) = _Fp_dot_old[_qp](i,j) / dp_old_cd;  // Previous Fp derivative
      }
    }
  }

  // Compute d(Fp^{-1})/dp at time n using: d(Fp^{-1})/dp = -Fp^{-1} * dFp/dp * Fp^{-1}
  RankTwoTensor dFpinv_dp_cd;
  dFpinv_dp_cd.zero();
  ...
  for (unsigned int i = 0; i < 3; ++i){
    for (unsigned int j = 0; j < 3; ++j){
      for (unsigned int k = 0; k < 3; ++k){
        for (unsigned int l = 0; l < 3; ++l){
          dFpinv_dp_cd(i, j) -= Fpinv_cd(i, k) * dFp_previous_dp(k, l) * Fpinv_cd(l, j);
        }
      }
    }
  }
  ...
  for (unsigned int i = 0; i < 3; ++i){
    for (unsigned int j = 0; j < 3; ++j){
      ...
      // Term 1: (1/(2*dt)) * ∂Fp^{n+1}_{ik}/∂p * Fpinv(k,j)
      for (unsigned int k = 0; k < 3; ++k){
        val += inv2dt_cd * dFp_current_dp(i, k) * Fpinv_cd(k, j);
      }
      
      // Term 2: (1/(2*dt)) * (Fp^{n+1}_{ik} - Fp^{n-1}_{ik}) * ∂(Fp^{-1}_{kj})/∂p
      for (unsigned int k = 0; k < 3; ++k){
        val += inv2dt_cd * deltaFp_cd(i, k) * dFpinv_dp_cd(k, j);
      }
...
void
ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit::computeQpPK2Stress()
{
  /* Evaluate Fp */
...
RankTwoTensor
ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit::computeQpFp()
{
  // //NOT FINISHED
  ...
  {
    // First: Compute beta using OLD eta (explicit)
    _dilatancy_function_beta[_qp] = _anand_param_go_mat * std::pow( 1 - _eta_old[_qp] / _anand_param_eta_cv_mat, _anand_param_p_mat );
    
    // Second: Compute shear rate using old values
    _shear_rate_nu[_qp] = _C_g[_qp] * std::pow(_B_breakagevar_old[_qp], _m1[_qp]) * std::pow(Tau_norm, _m2[_qp]) * N;
    
    // Third: Update eta using central difference with old beta
    _eta[_qp] = _eta_older[_qp] + 2.0 * _dilatancy_function_beta[_qp] * _C_g[_qp] * std::pow(_B_breakagevar_old[_qp], _m1[_qp]) * std::pow(Tau_norm, _m2[_qp]) * _dt;
  }
  else
  {
    _shear_rate_nu[_qp] = _C_g[_qp] * std::pow(_B_breakagevar_old[_qp], _m1[_qp]) * std::pow(Tau_norm, _m2[_qp]) * N;
  ...
  if (_use_dilatancy)
  {
    // Use beta computed from old eta (fully explicit)
  ...
  //Use Explicit Integration for Central Difference
  RankTwoTensor Fp_updated = _Fp_old[_qp] + _dt * Dp * _Fp_old[_qp];
...
void
ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit::computeQpTangentModulus(RankFourTensor & tangent, 
                                                                  Real I1, 
                                                                  Real I2, 
                                                                  Real xi, 
                                                                  RankTwoTensor Ee)
...
void 
ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit::computeDmatrix()
{
...
void
ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit::computeDeviatroicStrainRateTensor()
{
  //Compute D matrix
  computeDmatrix();
...
void 
ComputeLagrangianDamageBreakageStressPK2DiffusedExplicit::computestatedependentDp()
{
