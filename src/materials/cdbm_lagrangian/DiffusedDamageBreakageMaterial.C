...
#include "DiffusedDamageBreakageMaterial.h"
 ...
 */
registerMooseObject("farmsApp", DiffusedDamageBreakageMaterial);

InputParameters
DiffusedDamageBreakageMaterial::validParams()
{
  ...
  params.addRequiredCoupledVar("alpha_damagedvar_sys", "damage variable from system solve");
  return params;
}

DiffusedDamageBreakageMaterial::DiffusedDamageBreakageMaterial(const InputParameters & parameters)
  : Material(parameters),
  ...
  _C1_mat(declareProperty<Real>("C1")),
  _C2_mat(declareProperty<Real>("C2")),
  ...
  _alpha_damagedvar_sys(coupledValue("alpha_damagedvar_sys"))
{
...
//
void 
DiffusedDamageBreakageMaterial::initQpStatefulProperties()
{
...
void
DiffusedDamageBreakageMaterial::computeQpProperties()
{
  ...
  // Get shear_modulus_o value //this is initial shear modulus, it is not evolving with time
  const Real _shear_modulus_o = _use_shear_modulus_o_aux ? (*_shear_modulus_o_aux)[_qp] : _shear_modulus_o_value;
...
void
DiffusedDamageBreakageMaterial::updateCd()
{
...
void
DiffusedDamageBreakageMaterial::updateC1C2()
{
...
void
DiffusedDamageBreakageMaterial::updatexi0()
{
  // Get xi_0 value
  const Real _xi_0 = _use_xi0_aux ? (*_xi0_aux)[_qp] : _xi0_value;
  // Save xi_0 as material property
  _xi_0_mat[_qp] = _xi_0;
}


Real 
DiffusedDamageBreakageMaterial::updatedamage()
{
  //update alpha at current time
  Real alpha_damagedvar = _alpha_damagedvar_sys[_qp];
...
void
DiffusedDamageBreakageMaterial::updatebreakage()
{
  ...
  // Get Cd value based on results get from updateCd() function
  Real Cd = _Cd_mat[_qp];  
  // Get Cd value based on option
...
void 
DiffusedDamageBreakageMaterial::updatemodulus(Real alpha_updated)
{
...
void 
DiffusedDamageBreakageMaterial::computegammar()
{
...
void
DiffusedDamageBreakageMaterial::computecoefficients()
{
...
Real 
DiffusedDamageBreakageMaterial::alphacr_root1(Real xi) {
...
Real 
DiffusedDamageBreakageMaterial::alphacr_root2(Real xi) {
...
void
DiffusedDamageBreakageMaterial::computePrincipalStrainAndOrientation(
    RealVectorValue & strain_in_crack_dir)
{
