...
registerMooseObject("farmsApp", QuasiDynamicRSFv2);

InputParameters
QuasiDynamicRSFv2::validParams()
{ 
...
QuasiDynamicRSFv2::QuasiDynamicRSFv2(const InputParameters & parameters)
...
void
QuasiDynamicRSFv2::computeInterfaceTractionAndDerivatives()
{   
    // Get current inputs
    ...
    Real theta_dot = 1.0 - (theta_old * V_new / _L);
...
Real 
QuasiDynamicRSFv2::solveSlipRate(Real Tx, Real Ty, Real p_avg, Real theta_old, Real V_old)
{
    // Initial guess
    Real x = V_old;
    
    // Check if slip rate is very small
    Real test = _V_o * std::exp(1.0 / _a * (Tx / (Ty) - _f_o - _b * std::log(_V_o * theta_old / _L)));
    ...
    while (er > 1e-8 && iter < max_itr)
    {
        ...
        else
        {
            ...
            Func = Tx - (_nu * xo) - 
                   (Ty) * (_a * std::asinh(xo / (2.0 * _V_o) * 
                    std::exp((_f_o + _b * std::log(_V_o * theta_old / _L)) / _a)));
        ...
        else
        {
            ...
            {
                dFunc = -_nu / 2.0 - 
                        (Ty * _a * std::exp((_f_o + _b * std::log(_V_o * theta_old / _L)) / _a)) / 
                        (2.0 * _V_o * 
                         std::sqrt(1.0 + std::pow(xo * std::exp((_f_o + _b * std::log(_V_o * theta_old / _L)) / _a) / (2.0 * _V_o), 2)));
            }
...
Real 
QuasiDynamicRSFv2::computeSlip(Real slip_old, Real V_new, Real V_old)
{
...
Real 
QuasiDynamicRSFv2::computeStateVariable(Real theta_old, Real theta_dot, Real theta_dot_old)
{
