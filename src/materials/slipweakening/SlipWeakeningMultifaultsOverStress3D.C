...
#include "SlipWeakeningMultifaultsOverStress3D.h"
#include "InterfaceKernel.h"

registerMooseObject("farmsApp", SlipWeakeningMultifaultsOverStress3D);

InputParameters
SlipWeakeningMultifaultsOverStress3D::validParams()
{
...
SlipWeakeningMultifaultsOverStress3D::SlipWeakeningMultifaultsOverStress3D(const InputParameters & parameters)
...
void
SlipWeakeningMultifaultsOverStress3D::computeInterfaceTractionAndDerivatives()
{   
   ...
   Real tau_f = 0;
    ...
    //hard code nucleation process by overstress
    ...
    Real loc_y = 0;
    Real loc_z = -3000;
    Real overstress_pf = 1000 * 9.8 * abs(loc_z);
    Real overstress_sigma_dip = -2670 * 9.8 * abs(loc_z);
    Real overstress_b22 = 0.926793;
    Real overstress_sigma_normal = overstress_b22 * ( overstress_sigma_dip + overstress_pf ) - overstress_pf;
    Real overstress_val = 1.05 * mu_s * overstress_sigma_normal;
    Real patch = 1000;
    if ( (x_coord >= loc_x - patch / 2 ) && (x_coord <= loc_x + patch / 2 ) && (y_coord >= loc_y - patch / 2 ) && (y_coord <= loc_y + patch / 2 ) && (z_coord >= loc_z - patch / 2 ) && (z_coord <= loc_z + patch / 2 ) )
    {
      T1_o = overstress_val;
    }
