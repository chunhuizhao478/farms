...
#include "GeneralNonReflectDashpotBC_z.h"

registerMooseObject("farmsApp", GeneralNonReflectDashpotBC_z);

InputParameters
GeneralNonReflectDashpotBC_z::validParams()
{
  ...
  params.addCoupledVar("disp_y", "Displacement in the Z direction");
...
GeneralNonReflectDashpotBC_z::GeneralNonReflectDashpotBC_z(const InputParameters & parameters)
...
Real
GeneralNonReflectDashpotBC_z::computeQpJacobian()
{
...
Real
GeneralNonReflectDashpotBC_z::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _disp_x_id)
    return _test[_i][_qp] * _density[_qp] *
   ...
   return _test[_i][_qp] * _density[_qp] *
         (_p_wave_speed *  _d_disp_y_dot[_qp] * _normals[_qp](_disp_y_id) * _normals[_qp](_component) +
          _shear_wave_speed * ( _d_disp_y_dot[_qp] -  _d_disp_y_dot[_qp] * _normals[_qp](_disp_y_id) * _normals[_qp](_component)))* _phi[_j][_qp];
  return 0.0;
}
