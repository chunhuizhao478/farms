...
#include "MooseMesh.h"
#include "ADFarmsNonReflectDashpotBC.h"

registerADMooseObject("farmsApp", ADFarmsNonReflectDashpotBC);

InputParameters
ADFarmsNonReflectDashpotBC::validParams()
{
  InputParameters params = ADIntegratedBC::validParams();
  params.addClassDescription("Applies Lysmer damper in the normal and "
                             "tangential directions to soil boundary using automatic differentiation.");
  params += ADFarmsNonReflectDashpotBC::commonParameters();
...
InputParameters
ADFarmsNonReflectDashpotBC::commonParameters()
{
...
ADFarmsNonReflectDashpotBC::ADFarmsNonReflectDashpotBC(const InputParameters & parameters)
...
ADReal
ADFarmsNonReflectDashpotBC::computeQpResidual()
{
  std::vector<ADReal> vel(3, 0.0);
  ADReal accel(0.0);

  ADReal normal_vel = 0.0;
  for (unsigned int i = 0; i < _ndisp; ++i)
  {
