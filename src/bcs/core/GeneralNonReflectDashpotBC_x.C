...
#include "GeneralNonReflectDashpotBC_x.h"

registerMooseObject("farmsApp", GeneralNonReflectDashpotBC_x);

InputParameters
GeneralNonReflectDashpotBC_x::validParams()
{
...
GeneralNonReflectDashpotBC_x::GeneralNonReflectDashpotBC_x(const InputParameters & parameters)
...
Real
GeneralNonReflectDashpotBC_x::computeQpResidual()
{
  std::vector<Real> velocity(3, 0.0);
  velocity[0] = _u_dot[_qp];
...
Real
GeneralNonReflectDashpotBC_x::computeQpJacobian()
{
...
Real
GeneralNonReflectDashpotBC_x::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _disp_y_id)
    return _test[_i][_qp] * _density[_qp] *
