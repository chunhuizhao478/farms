 ...
 */
#include "ElkADDynamicDarcyFlow.h"
#include "SubProblem.h"

registerMooseObject("farmsApp", ElkADDynamicDarcyFlow);

InputParameters ElkADDynamicDarcyFlow::validParams()
{
...
ElkADDynamicDarcyFlow::ElkADDynamicDarcyFlow(const InputParameters & parameters):ADKernel(parameters),
    ...
    _kappa(getADMaterialProperty<Real>("permeability")),
...
ADReal
ElkADDynamicDarcyFlow::computeQpResidual()
{
    ...
    return _test[_i][_qp]*( _rhof[_qp]*as + _rhof[_qp]*_taut[_qp]/_nf[_qp]*af + _muf[_qp]/_kappa[_qp]*vf ); 
}