...
#include "PoroMechanicsCoupling2.h"
...
registerMooseObject("farmsApp", PoroMechanicsCoupling2);

InputParameters
PoroMechanicsCoupling2::validParams()
{
...
PoroMechanicsCoupling2::PoroMechanicsCoupling2(const InputParameters & parameters)
  : Kernel(parameters),
    ...
    _component(getParam<unsigned int>("component"))
{
  if (_component >= _mesh.dimension())
    mooseError("PoroMechanicsCoupling2: component should not be greater than the mesh dimension\n");
}

Real
PoroMechanicsCoupling2::computeQpResidual()
{
  return -  _porepressure[_qp] * _grad_test[_i][_qp](_component);
}

Real
PoroMechanicsCoupling2::computeQpJacobian()
{
...
Real
PoroMechanicsCoupling2::computeQpOffDiagJacobian(unsigned int jvar)
{
