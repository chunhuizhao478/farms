...
DynamicDarcyFlow2::DynamicDarcyFlow2(const InputParameters & parameters)
  : TimeKernel(parameters),
    ...
    _time_integrator(_sys.getTimeIntegrator(_us_var_num))  // Fixed: pass variable number
{
...
DynamicDarcyFlow2::computeQpResidual()
{
  ...
  Real rhoa = (_taut[_qp] - 1.0) * _nf[_qp];
  return _test[_i][_qp] * _rhof[_qp] * ((*_us_dot_dot)[_qp] + 
                                        (*_u_dot_factor)[_qp] * (1.0 + rhoa / _nf[_qp]) / _nf[_qp] + 
                                        1.0 / _K[_qp] / _rhof[_qp] * _u[_qp]);
}
...
DynamicDarcyFlow2::computeQpJacobian()
{
  ...
  Real rhoa = (_taut[_qp] - 1.0) * _nf[_qp];
  return _test[_i][_qp] * _rhof[_qp] * ((*_du_dot_du)[_qp] * (1.0 + rhoa / _nf[_qp]) / _nf[_qp] +
                                        1.0 / _K[_qp] / _rhof[_qp]) * _phi[_j][_qp];
}
