...
CoupledSolidmatrixInertia::computeQpResidual()
{
  ...
  // Fixed the logic flow and syntax
  if (_time_integrator.isLumped())
  {
    Real residual = _test[_i][_qp] * _rhof[_qp] / _dt / _dt;
    // Apply lumping to local residual
    for (unsigned int i = 0; i < _test.size(); ++i)
      this->_local_re(i) *= (_us[i] - 2.0 * _us_old[_i] + _us_older[_i]);
    return residual;
  }
  else
  {
    return _test[_i][_qp] * _rhof[_qp] * (_us[_i] - 2.0 * _us_old[_i] + _us_older[_i]) / _dt / _dt;
  }
}
