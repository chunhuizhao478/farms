...
#include "TotalStressDivergenceTensor.h"
...
registerMooseObject("farmsApp", TotalStressDivergenceTensor);

InputParameters
TotalStressDivergenceTensor::validParams()
{
  ...
  params.addCoupledVar("porepressure", "The pore pressure variable");
  return params;
}

TotalStressDivergenceTensor::TotalStressDivergenceTensor(const InputParameters & parameters)
  : JvarMapKernelInterface<ALEKernel>(parameters),
    ...
    _stress_off_diag_jacobian(getMaterialPropertyByName<RankTwoTensor>(_base_name + "stress_off_diag_jacobian"))
{
...
void
TotalStressDivergenceTensor::initialSetup()
{
...
void
TotalStressDivergenceTensor::computeResidual()
{
...
Real
TotalStressDivergenceTensor::computeQpResidual()
{
...
void
TotalStressDivergenceTensor::computeJacobian()
{
  if (_volumetric_locking_correction)
  {
    computeAverageGradientTest();
...
void
TotalStressDivergenceTensor::computeOffDiagJacobian(const unsigned int jvar)
{
  if (_volumetric_locking_correction)
  {
    computeAverageGradientPhi();
...
Real
TotalStressDivergenceTensor::computeQpJacobian()
{
  if (_use_finite_deform_jacobian)
...
Real
TotalStressDivergenceTensor::computeQpOffDiagJacobian(unsigned int jvar)
{

  // Check for pore pressure coupling first
  if (isCoupled("porepressure") && jvar == _pore_pressure_var)
    return computeQpOffDiagJacobianPorePressure(jvar);  
...
void
TotalStressDivergenceTensor::computeFiniteDeformJacobian()
{
...
void
TotalStressDivergenceTensor::computeAverageGradientTest()
{
...
void
TotalStressDivergenceTensor::computeAverageGradientPhi()
{
...
Real
TotalStressDivergenceTensor::computeQpOffDiagJacobianPorePressure(unsigned int jvar)
{
  // Off-diagonal Jacobian: derivative of residual w.r.t. pore pressure
  if (jvar != _pore_pressure_var)
    return 0.0;

  // This computes: grad(test)_component : d(σ)/dp * phi(p)
  // For small strain: ∂R_α/∂p = ∂ψ/∂x_i * ∂σ_iα/∂p * φ_p
  
  Real jacobian = _stress_off_diag_jacobian[_qp].row(_component) * _grad_test[_i][_qp] * _phi[_j][_qp];
  
  // Add volumetric locking correction if needed
  if (_volumetric_locking_correction)
  {
    jacobian += _stress_off_diag_jacobian[_qp].trace() / 3.0 *
                (_avg_grad_test[_i][_component] - _grad_test[_i][_qp](_component)) * _phi[_j][_qp];
  }
