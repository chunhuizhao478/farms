...
#include "BreakageVarForcingFuncDevOld.h"

registerMooseObject("farmsApp", BreakageVarForcingFuncDevOld);

InputParameters
BreakageVarForcingFuncDevOld::validParams()
{
...
BreakageVarForcingFuncDevOld::BreakageVarForcingFuncDevOld(const InputParameters & parameters)
 : Kernel(parameters),
...
Real
BreakageVarForcingFuncDevOld::computeQpResidual()
{ 
...
Real
BreakageVarForcingFuncDevOld::computeQpJacobian()
{
  return 0.0;
}
...
Real 
BreakageVarForcingFuncDevOld::computeAlphaCr(Real xi)
{
  Real alphacr;
  if ( xi < _xi_0 )
  {
  ...
  else if ( xi > _xi_0 && xi <= _xi_1 )
  {
    ...
    alphacr = ((xi*7.8855e6-1.131530154260151e4*xi*1.5e3-1.131530154260151e4*2.45e3-1.131530154260151e4*pow(xi,3)*2.5e2+pow(xi,3)*1.31425e6+sqrt((1.131530154260151e4*5.257e3-7.7836049e7)*(xi*-1.26e3-pow(xi,2)*2.1e3+pow(xi,3)*4.9e2+pow(xi,4)*3.0e2+pow(xi,6)*2.5e1+3.441e3)*(-7.936381962191076e-2))*2.51e+2+1.287965e7)*(-2.51e2/2.0))/(xi*8.172785145e9-1.131530154260151e+4*xi*5.51985e5+1.131530154260151e4*1.36682e5-1.131530154260151e4*pow(xi,2)*3.94275e+5+pow(xi,2)*5.837703675e9-2.023737274e9);
  }
  else if ( xi > _xi_1 && xi <= _xi_max )
  {
    ...
    alphacr = 6.408e10/(xi*(1.131530154260151e4*6.382470119521912e6-3.355264541832669e10)+1.131530154260151e4*8.935458167330677e6-4.697370358565737e10);
  }
  else
  {
    std::cout<<"xi: "<<xi<<std::endl;
