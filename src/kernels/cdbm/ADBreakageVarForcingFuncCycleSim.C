...
#include "ADBreakageVarForcingFuncCycleSim.h"

registerMooseObject("farmsApp", ADBreakageVarForcingFuncCycleSim);

InputParameters
ADBreakageVarForcingFuncCycleSim::validParams()
{
...
ADBreakageVarForcingFuncCycleSim::ADBreakageVarForcingFuncCycleSim(const InputParameters & parameters)
 : ADKernel(parameters),
...
ADReal
ADBreakageVarForcingFuncCycleSim::computeQpResidual()
{ 
    ...
    if ( xi >= _xi_0 && xi <= _xi_max ){
        return -1.0 * C_B * Prob * (1-B) * I2 * (xi - _xi_0); //could heal if xi < xi_0
    }
    else if ( xi < _xi_0 && xi >= _xi_min ){
        return -1.0 * C_BH * I2 * ( xi - _xi_0 );
        //return 0.0;
    }
    else{
...
ADReal 
ADBreakageVarForcingFuncCycleSim::computeAlphaCr(ADReal xi)
{
  ADReal alphacr;
  if ( xi < _xi_0 )
  {
