...
registerMooseObject("farmsApp", FluidSolidCoupling);

InputParameters
FluidSolidCoupling::validParams()
{
...
FluidSolidCoupling::FluidSolidCoupling(const InputParameters & parameters)
  : Kernel(parameters),
    _ndisp(coupledComponents("displacements")),
    _disp_var(_ndisp),
    _I1(getMaterialProperty<Real>(getParam<std::string>("base_name") + "first_elastic_strain_invariant")),
    _I1_old(getMaterialPropertyOld<Real>(getParam<std::string>("base_name") + "first_elastic_strain_invariant")),
...
Real
FluidSolidCoupling::computeQpResidual()
{
...
Real
FluidSolidCoupling::computeQpJacobian()
{
  return 0.0;
}

Real 
FluidSolidCoupling::computeQpOffDiagJacobian(unsigned int jvar)
{
  for (unsigned int i = 0; i < _ndisp; ++i)
  {
    if (jvar == _disp_var[i])
