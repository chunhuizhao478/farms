...
#include "ADCoupledReaction.h"

registerADMooseObject("farmsApp", ADCoupledReaction);

InputParameters
ADCoupledReaction::validParams()
{
  InputParameters params = ADKernel::validParams();
  params.addClassDescription(
      "Implements a simple consuming reaction term with weak form $(\\psi_i, \\lambda u_h)$ using automatic differentiation.");
...
ADCoupledReaction::ADCoupledReaction(const InputParameters & parameters)
  : ADKernel(parameters), 
...
ADReal
ADCoupledReaction::computeQpResidual()
{
