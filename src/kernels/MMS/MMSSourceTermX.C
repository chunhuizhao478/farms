#include "MMSSourceTermX.h"
#include <cmath>

registerMooseObject("farmsApp", MMSSourceTermX);

InputParameters
MMSSourceTermX::validParams()
{
  InputParameters params = BodyForce::validParams();
  params.addClassDescription("Implements the source term for the MMS in x-direction (continuous domain)");
...
MMSSourceTermX::MMSSourceTermX(const InputParameters & parameters) :
...
Real
MMSSourceTermX::computeQpResidual()
{
...
Real
MMSSourceTermX::spatialFunction(Real x, Real y) const
{
...
Real
MMSSourceTermX::temporalFunction(Real t) const
{
    return (_delta/2.0) * std::exp(( t - _tbar)/_tw);
}

Real
MMSSourceTermX::temporalDerivative(Real t) const
{
    return (_delta/2.0) * std::exp(( t - _tbar)/_tw)/_tw;
}

Real
MMSSourceTermX::temporalSecondDerivative(Real t) const
{
...
Real
MMSSourceTermX::sourceTerm(Real x, Real y, Real t) const {
  // Spatial function and derivatives
  Real width = _R_m / 2.0;
  ...
  // Stress divergence
  Real div_sigma_x = 
    _lambda * K * (d2_spatial_phi_dx2 + d2_spatial_phi_dy2) +  2.0 * _mu * K * d2_spatial_phi_dx2;
  ...
  // Final source term
  return accel_term - div_sigma_x - div_sigma_xy;
}