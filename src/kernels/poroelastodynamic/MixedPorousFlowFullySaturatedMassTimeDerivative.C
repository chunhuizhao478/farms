...
#include "MixedPorousFlowFullySaturatedMassTimeDerivative.h"

#include "MooseVariable.h"

registerMooseObject("PorousFlowApp", MixedPorousFlowFullySaturatedMassTimeDerivative);

InputParameters
MixedPorousFlowFullySaturatedMassTimeDerivative::validParams()
{
...
MixedPorousFlowFullySaturatedMassTimeDerivative::MixedPorousFlowFullySaturatedMassTimeDerivative(
  ...
  : TimeKernel(parameters),
    ...
    _dtemperature_dvar(_includes_thermal ? &getMaterialProperty<std::vector<Real>>(
                                         ...
                                         : nullptr),
    _darcy_velocity_divergence(_includes_mechanical
                     ? &getMaterialProperty<Real>("PorousFlow_divergence_darcy_velocity_qp")
                     : nullptr),
    _ddarcy_velocity_divergence(_includes_mechanical ? &getMaterialProperty<std::vector<RealGradient>>(
                                                  "dPorousFlow_divergence_darcy_velocity_qp")
                                            : nullptr),
                                            ...
                                            : nullptr)
{
  if (_dictator.numComponents() != 1 || _dictator.numPhases() != 1)
    mooseError("MixedPorousFlowFullySaturatedMassTimeDerivative is only applicable to single-phase, "
...
Real
MixedPorousFlowFullySaturatedMassTimeDerivative::computeQpResidual()
{
  ...
  if (_includes_mechanical)
    volume = -_biot_coefficient * (*_strain_rate)[_qp] - (*_darcy_velocity_divergence)[_qp];
...
Real
MixedPorousFlowFullySaturatedMassTimeDerivative::computeQpJacobian()
{
...
Real
MixedPorousFlowFullySaturatedMassTimeDerivative::computeQpOffDiagJacobian(unsigned int jvar)
{
...
Real
MixedPorousFlowFullySaturatedMassTimeDerivative::computeQpJac(unsigned int pvar)
{
  const unsigned phase = 0;
  Real volume = (_pp[_qp][phase] - _pp_old[_qp][phase]) / _dt / _biot_modulus[_qp];
  ...
  if (_includes_mechanical)
  {
    ...
    dvolume += -(_biot_coefficient * (*_dstrain_rate_dvar)[_qp][pvar]- (*_ddarcy_velocity_divergence)[_qp][pvar] ) * _grad_phi[_j][_qp]; 
