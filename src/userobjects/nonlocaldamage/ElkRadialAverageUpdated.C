...
/*
Note: Currently this only works for maximum cracks for each element = 1
Created By Chunhui Zhao, Sep 28, 2024
*/
...
registerMooseObject("farmsApp", ElkRadialAverageUpdated);

// specialization for PointListAdaptor<ElkRadialAverageUpdated::QPData>
template <>
const Point &
PointListAdaptor<ElkRadialAverageUpdated::QPData>::getPoint(const ElkRadialAverageUpdated::QPData & item) const
{
  return item._q_point;
}

InputParameters
ElkRadialAverageUpdated::validParams()
{
  ...
  /*--------------------------------------------------------------------------------------------*/
  params.addRequiredParam<Real>("length_scale", "characteristic length scale, material property");
  /*--------------------------------------------------------------------------------------------*/
...
ElkRadialAverageUpdated::ElkRadialAverageUpdated(const InputParameters & parameters)
...
void
ElkRadialAverageUpdated::initialSetup()
{
...
void
ElkRadialAverageUpdated::initialize()
{
  _qp_data.clear();
}

void
ElkRadialAverageUpdated::execute()
{
...
void
ElkRadialAverageUpdated::finalize()
{
  ...
  ThreadedElkRadialAverageLoopUpdated rgcl(*this);

  // run threads
  auto local_range_begin = _qp_data.begin();
  auto local_range_end = local_range_begin;
  std::advance(local_range_end, local_size);
  Threads::parallel_reduce(QPDataRangeUpdated(local_range_begin, local_range_end), rgcl);
}

void
ElkRadialAverageUpdated::threadJoin(const UserObject & y)
{
  const auto & uo = static_cast<const ElkRadialAverageUpdated &>(y);
  _qp_data.insert(_qp_data.begin(), uo._qp_data.begin(), uo._qp_data.end());
  _average.insert(uo._average.begin(), uo._average.end());
}

void
ElkRadialAverageUpdated::meshChanged()
{
...
void
ElkRadialAverageUpdated::updateCommunicationLists()
{
...
StandardType<ElkRadialAverageUpdated::QPData>::StandardType(const ElkRadialAverageUpdated::QPData * example)
{
  // We need an example for MPI_Address to use
  static const ElkRadialAverageUpdated::QPData p;
  if (!example)
    example = &p;
  ...
  libmesh_call_mpi(MPI_Get_address(const_cast<ElkRadialAverageUpdated::QPData *>(example), &start));
  ...
  libmesh_call_mpi(MPI_Type_create_resized(tmptype, 0, sizeof(ElkRadialAverageUpdated::QPData), &_datatype));
...
StandardType<ElkRadialAverageUpdated::QPData>::StandardType(const StandardType<ElkRadialAverageUpdated::QPData> & t)
