...
#include "ThreadedElkRadialAverageLoop.h"
...
registerMooseObject("farmsApp", ElkRadialAverage);

// specialization for PointListAdaptor<ElkRadialAverage::QPData>
template <>
const Point &
PointListAdaptor<ElkRadialAverage::QPData>::getPoint(const ElkRadialAverage::QPData & item) const
{
  return item._q_point;
}

InputParameters
ElkRadialAverage::validParams()
{
...
void
ElkRadialAverage::initialize()
{
  _qp_data.clear();
}

void
ElkRadialAverage::execute()
{
...
ElkRadialAverage::finalize()
{
  ...
  ThreadedElkRadialAverageLoop rgcl(*this);
...
void
ElkRadialAverage::threadJoin(const UserObject & y)
{
  const auto & uo = static_cast<const ElkRadialAverage &>(y);
  _qp_data.insert(_qp_data.begin(), uo._qp_data.begin(), uo._qp_data.end());
  _average.insert(uo._average.begin(), uo._average.end());
}

void
ElkRadialAverage::meshChanged()
{
...
void
ElkRadialAverage::updateCommunicationLists()
{
...
StandardType<ElkRadialAverage::QPData>::StandardType(const ElkRadialAverage::QPData * example)
{
  // We need an example for MPI_Address to use
  static const ElkRadialAverage::QPData p;
  if (!example)
    example = &p;
  ...
  libmesh_call_mpi(MPI_Get_address(const_cast<ElkRadialAverage::QPData *>(example), &start));
  ...
  libmesh_call_mpi(MPI_Type_create_resized(tmptype, 0, sizeof(ElkRadialAverage::QPData), &_datatype));
...
StandardType<ElkRadialAverage::QPData>::StandardType(const StandardType<ElkRadialAverage::QPData> & t)
