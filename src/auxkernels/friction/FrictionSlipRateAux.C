...
#include "FrictionSlipRateAux.h"
#include "BrentRootFinder.h"
#include <cmath>

registerMooseObject("farmsApp", FrictionSlipRateAux);

InputParameters
FrictionSlipRateAux::validParams()
{
  InputParameters params = AuxKernel::validParams();
  params.addClassDescription(
      "Computes slip rate V from quasi-dynamic traction balance using Brent's method. "
      "Solves: τ = σn * f(V, θ) + η * V for V. "
      "Designed for Friction SubApp in MultiApp SEAS architecture.");

  params.addRequiredCoupledVar("traction", "The elastic traction variable (Pa)");
  params.addRequiredCoupledVar("state_variable", "The state variable θ (s)");
...
FrictionSlipRateAux::FrictionSlipRateAux(const InputParameters & parameters)
...
{
  if (_a <= 0.0)
    mooseError("Rate-state parameter 'a' must be positive");
  if (_b < 0.0)
    mooseError("Rate-state parameter 'b' must be non-negative");
  if (_Dc <= 0.0)
    mooseError("Critical slip distance 'Dc' must be positive");
  if (_V0 <= 0.0)
    mooseError("Reference velocity 'V0' must be positive");
  if (_eta <= 0.0)
    mooseError("Radiation damping 'eta' must be positive");
}

Real
FrictionSlipRateAux::computeValue()
{
  // Total traction = elastic + pre-stress
...
Real
FrictionSlipRateAux::solveSlipRate(Real tau, Real theta) const
{
...
Real
FrictionSlipRateAux::frictionCoefficient(Real V, Real theta) const
{
