...
/**
 * ComputeDamageBreakageStressv2 put everything inside the computeQpstress without defining
 * additional functions
 
 */
class ComputeDamageBreakageStressv2 : public ComputeDamageBreakageStressBase
{
public:
  static InputParameters validParams();

  ComputeDamageBreakageStressv2(const InputParameters & parameters);
...
protected:
  ...
  /// Properties the published cdbm@99b41e9c build declared in
  /// ComputeGeneralDamageBreakageStressBase. This branch's copy of that shared base does
  /// not declare them; declaring them here keeps this material bit-compatible with the
  /// published run without changing what any other CDBM material declares or outputs.
  /// Must be declared BEFORE the _old getters below: declareProperty() has to run before
  /// getMaterialPropertyOldByName() for the same name, and members initialise in
  /// declaration order.
