...
class PoroRateStateFrictionLaw2DAsBC : public PoroCZMComputeLocalTractionTotalBaseRSF2D
{
public:
  static InputParameters validParams();
  PoroRateStateFrictionLaw2DAsBC(const InputParameters & parameters);
...
protected:
  ...
  const VariableValue & _reaction_visco_rsf_x;
  const VariableValue & _reaction_visco_rsf_y;
  const VariableValue & _reaction_visco_rsf_neighbor_x;
  const VariableValue & _reaction_visco_rsf_neighbor_y;
