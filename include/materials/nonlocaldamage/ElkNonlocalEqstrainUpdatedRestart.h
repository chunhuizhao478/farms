...
update it using "ElkRadialAverageUpdated"
...
class ElkNonlocalEqstrainUpdatedRestart : public Material
{
public:
  static InputParameters validParams();

  ElkNonlocalEqstrainUpdatedRestart(const InputParameters & parameters);
...
protected:
  ...
  const ElkRadialAverageUpdated::Result & _average;
  ElkRadialAverageUpdated::Result::const_iterator _average_eqstrain_nonlocal;

  // Pointer to last element for comparison for speed
  const Elem * _prev_elem;

  const MaterialProperty<Real> & _eqstrain_nonlocal_initial;
