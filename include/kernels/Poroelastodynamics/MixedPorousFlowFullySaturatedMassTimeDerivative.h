 ...
 */
class MixedPorousFlowFullySaturatedMassTimeDerivative : public TimeKernel
{
public:
  static InputParameters validParams();

  MixedPorousFlowFullySaturatedMassTimeDerivative(const InputParameters & parameters);
...
protected:
  ...
  const MaterialProperty<std::vector<Real>> * const _dtemperature_dvar;
   ...
   /// Strain rate
  const MaterialProperty<Real> * const _darcy_velocity_divergence;

  /// Derivative of strain rate wrt the PorousFlow variables
  const MaterialProperty<std::vector<RealGradient>> * const _ddarcy_velocity_divergence;
