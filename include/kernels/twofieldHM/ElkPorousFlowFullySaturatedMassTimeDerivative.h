...
public:
  ...
  ElkPorousFlowFullySaturatedMassTimeDerivative(const InputParameters & parameters);
...
protected:
  ...
  /// Toggle to use material property for Biot coefficient
  const bool _use_damaged_biot;

  /// Biot coefficient material property (valid if _use_damaged_biot)
  const MaterialProperty<Real> * _biot_coefficient_mp;
  ...
  inline Real biot() const
  {
    return _use_damaged_biot ? (*_biot_coefficient_mp)[_qp] : _biot_coefficient_const;
  }
};
