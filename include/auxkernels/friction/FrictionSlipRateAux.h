...
/**
 * FrictionSlipRateAux computes slip rate V from the quasi-dynamic traction balance
 * equation using Brent's method for robust root finding.
 *
 * Solves: τ = σn * f(V, θ) + η * V
 *
 * where:
 *   τ = elastic traction (from MainApp via Transfer)
 ...
 * fault mesh.
 */
class FrictionSlipRateAux : public AuxKernel
{
public:
  static InputParameters validParams();

  FrictionSlipRateAux(const InputParameters & parameters);
...
protected:
  ...
  /// Elastic traction (from MainApp via Transfer)
